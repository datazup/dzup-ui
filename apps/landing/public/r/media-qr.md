...
A "continue on mobile" card built around DzQRCode — a token-styled SVG code with a centered brand mark via the #logo slot, error-level H, a pairing code, and an expire/refresh status flow.
