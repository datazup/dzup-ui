...
A ranked DzList where each row pairs a rank badge + avatar prefix with a DzAnimatedNumber score and a tone-coded delta tag; a refresh button mutates the values so the figures re-animate live.
