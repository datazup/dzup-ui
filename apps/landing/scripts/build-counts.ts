...
const PRO_SNAPSHOT_PATH = resolve(LANDING_ROOT, 'src', 'generated', 'pro-showcase.snapshot.json')
...
/**
 * Refresh the README's product-truth region from the same generated Core counts
 * and public-safe Pro snapshot used by the landing. The checked-in README stays
 * useful on Git hosts while its current numbers remain generator-owned.
 */
async function rewriteReadme(counts: Counts): Promise<void> {
  const snapshot = JSON.parse(await readFile(PRO_SNAPSHOT_PATH, 'utf8')) as ProSnapshot
  const start = '<!-- claims:generated:start -->'
  const end = '<!-- claims:generated:end -->'
  const current = await readFile(README_PATH, 'utf8')
  const pattern = new RegExp(`${start}[\\s\\S]*?${end}`)
  if (!pattern.test(current)) {
    throw new Error('README.md: generated claims region is missing')
  }

  const region = `${start}
**Open-source Vue 3 component library — ${counts.catalogComponents} catalog components, including ${counts.documentedComponents} with dedicated Storybook pages.**

> dzup-ui Pro ${snapshot.package.version} publishes ${snapshot.counts.publishedComponents} enterprise components across ${snapshot.counts.families} families. Its built Storybook contains ${snapshot.counts.stories} stories; ${snapshot.counts.dedicatedDocumentationPages} exported components have dedicated documentation pages, while compound parts are documented through their parent.
...
${end}`
  const next = current.replace(pattern, region)
  if (next !== current)
    await writeFile(README_PATH, next)
  console.log(`▸ README product claims rewritten from generated evidence${next === current ? ' (already current)' : ''}`)
}
