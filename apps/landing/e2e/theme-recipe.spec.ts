...
test('ThemeRecipeV1 persists, follows system mode, shares, resets, and remains accessible', async ({ page }) => {
  ...
  // `textContent()` is `string | null`; assert it rather than feeding null to
  // JSON.parse, which would throw "null is not valid JSON" and report as a parse
  // failure instead of the real cause — the export block never rendering.
  const exportedText = await page.getByTestId('theme-recipe-export').textContent()
  expect(exportedText, 'the serialized recipe must render').not.toBeNull()
