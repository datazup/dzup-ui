...
import { DzOdometer, vReveal } from '../../motion/index.ts'

// StatsSection (docs/landing-v2.md TASK-LV2-08) — the v1 SocialProof strip
// (which /classic keeps) with DzOdometer digits: each figure ROLLS to its
// target left-to-right on scroll-in instead of tweening, and each tile blurs
// in with a small stagger (v-reveal.blur — this section owns its entrance, so
// HomePage gives it a plain wrapper, see the choreography table there).
// Everything else — the honesty rules, the CLS reservation, the live-metric
// degradation — is v1's, verbatim:
//
    ...
    <div class="proof-inner">
      ...
      >
        <span class="stat-value" :style="{ '--reserve': reserve(FACTS.freeComponents) }">
          <DzOdometer :value="FACTS.freeComponents" :duration="1200" />
        </span>
      ...
      >
        <span class="stat-value" :style="{ '--reserve': reserve(FACTS.families) }">
          <DzOdometer :value="FACTS.families" :duration="1000" />
        </span>
      ...
      >
        <span class="stat-value" :style="{ '--reserve': reserve(PRO_FACTS.published) }">
          <DzOdometer :value="PRO_FACTS.published" :duration="1200" />
        </span>
      ...
      >
        <span class="stat-value" :style="{ '--reserve': reserve(githubStars) }">
          ...
          <DzOdometer v-if="githubStars !== null" :value="githubStars" :duration="1400" />
        </span>
      ...
      >
        <span class="stat-value" :style="{ '--reserve': reserve(npmDownloads) }">
          <DzOdometer v-if="npmDownloads !== null" :value="npmDownloads" :duration="1600" />
