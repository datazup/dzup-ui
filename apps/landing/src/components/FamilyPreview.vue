  ...
  <div class="preview">
    ...
    <template v-else-if="name === 'Forms'">
      <div class="row row--between">
        <DzText size="sm">Email alerts</DzText>
        <DzSwitch v-model="on" size="sm" aria-label="demo" />
      </div>
      <DzCheckbox v-model="checked">Subscribe to updates</DzCheckbox>
    ...
    <template v-else-if="name === 'Cards'">
      <div class="mini-card">
        ...
        <div>
          <DzText size="sm" weight="medium" as="div">Acme Inc.</DzText>
          <DzText size="xs" tone="muted" as="div">Team · 12 seats</DzText>
        </div>
    ...
    <template v-else-if="name === 'Data'">
      <div class="row">
        <DzTag tone="primary" size="sm">Vue</DzTag>
        <DzTag tone="success" size="sm">TS</DzTag>
        <DzTag tone="info" size="sm">OKLCH</DzTag>
      </div>
    ...
    <template v-else-if="name === 'Feedback'">
      <div class="row">
        <DzBadge variant="subtle" tone="success" size="sm">Passed</DzBadge>
        <DzBadge variant="subtle" tone="warning" size="sm">Pending</DzBadge>
        <DzBadge variant="subtle" tone="danger" size="sm">Failed</DzBadge>
      </div>
      <div class="row">
        <DzSpinner size="sm" tone="primary" />
        <DzText size="sm" tone="muted">Loading…</DzText>
      </div>
    </template>
    ...
    <template v-else-if="name === 'Navigation'">
      ...
      <div class="crumbs">
        <DzText size="xs" tone="muted">Home</DzText>
        <span class="crumb-sep">/</span>
        <DzText size="xs" tone="muted">Projects</DzText>
        <span class="crumb-sep">/</span>
        <DzText size="xs">Web</DzText>
      </div>
    </template>

    <template v-else-if="name === 'Overlays'">
      <div class="row row--between">
        <DzText size="sm">Quick actions</DzText>
        <DzKbd :keys="['cmd', 'K']" size="sm" />
      </div>
      <div class="popover-mock">
        <DzText size="xs" tone="muted" as="div">Tooltip · popover · dialog</DzText>
      </div>
    </template>
    ...
    <template v-else-if="name === 'Typography'">
      <DzHeading :level="3" size="md" weight="semibold">Type that scales</DzHeading>
      <DzText size="sm" tone="muted">Headings, text, code and kbd on one fluid scale.</DzText>
      <DzRating v-model="rating" :count="5" />
    </template>

    <template v-else>
      <DzText size="sm" tone="muted">{{ name }}</DzText>
    </template>
  </div>
</template>
