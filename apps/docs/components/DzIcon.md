---
title: "DzIcon"
description: "Icon wrapper component with consistent sizing and accessibility."
outline: [2, 3]
---
...
Icon wrapper component with consistent sizing and accessibility.

- **Family:** Media
- **Source:** `packages/core/src/components/media/DzIcon.vue` · types `packages/core/src/components/media/DzIcon.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzIcon } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/media`
- **Risk tier:** A · **Status:** stable

## Intent and selection guidance

**Not declared.** `DzIcon` declares no `@intent` block in its source header, so
...
## Usage (verbatim story source from `packages/core/stories/media/DzIcon.stories.ts`, story `Default`)
...
Editable, running the **Size Gallery** story from `packages/core/stories/media/DzIcon.stories.ts` (lines 83–113) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzIcon" />

## Variants and controlled state

**Not declared.** `DzIcon` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzIcon` has no `DzIcon.anatomy.ts`, so
...
**Not declared.** `DzIcon` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzIcon` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzIcon` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/media/DzIcon.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/media/DzIcon.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/media.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/media/DzIcon.stories.ts` |
