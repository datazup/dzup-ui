...
description: "Coordinator pattern pill for team runs."
outline: [2, 3]
---
...
Coordinator pattern pill for team runs.

- **Family:** Feedback
- **Source:** `packages/core/src/components/feedback/GovernanceBadge.vue` · types `packages/core/src/components/feedback/GovernanceBadge.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { GovernanceBadge } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/feedback`
- **Risk tier:** A · **Status:** stable
- **Taxonomy:** variant: `solid` `outline` `subtle` · size: `icon` `xs` `sm` `md` `lg` `xl`

## Intent and selection guidance

**Not declared.** `GovernanceBadge` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `pattern` | `CoordinatorPattern` | yes | — | Coordinator pattern governing the team run |
| `size` | `CanonicalSize \| undefined` | no | `"md"` | Badge size |
| `variant` | `BadgeVariant \| undefined` | no | `"solid"` | Visual style variant |

## Slots (1)

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `{ pattern: CoordinatorPattern; label: string; }` | Override the human-readable label (defaults to title-cased pattern name) |

## Usage (from `packages/core/stories/_app-specific/GovernanceBadge.stories.ts`, story `Default`)

```vue
<GovernanceBadge v-bind="args" />
```

## Playground

Editable, running the **Pattern Gallery** story from `packages/core/stories/_app-specific/GovernanceBadge.stories.ts` (lines 79–92) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="GovernanceBadge" />

## Variants and controlled state

**Not declared.** `GovernanceBadge` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `GovernanceBadge` has no `DzGovernanceBadge.anatomy.ts`, so
...
**Not declared.** `GovernanceBadge` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `GovernanceBadge` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `GovernanceBadge` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/feedback/GovernanceBadge.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/feedback/GovernanceBadge.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/_app-specific/GovernanceBadge.stories.ts` |
