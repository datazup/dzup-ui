...
**Not declared.** `DzCode` declares no `@intent` block in its source header, so
...
`packages/core/stories/typography/DzCode.stories.ts`: `Default`, `Variant Gallery`, `Inline Code in Paragraph`, `Block Code`, `With Language Hint`, `DzCode vs DzCodeBlock`, `Dark Mode Preview`, `Accessibility: Semantic Elements`, `Real World: Installation Guide`.
...
Declared in `packages/core/src/components/typography/DzCode.anatomy.ts`.
