...
- **Family:** Media
- **Source:** `packages/core/src/components/media/DzAvatar.vue` · types `packages/core/src/components/media/DzAvatar.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzAvatar } from '@dzup-ui/core'`
...
**Not declared.** `DzAvatar` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `alt` | `string \| undefined` | no | — | Alt text for the avatar image |
...
| `fallback` | `string \| undefined` | no | — | Fallback text (typically initials) when image is not available |
| `id` | `string \| undefined` | no | — | Unique element ID (prefer `useId()` from Vue 3.5 when auto-generated) |
| `shape` | `AvatarShape \| undefined` | no | `"circle"` | Avatar shape |
...
| --- | --- | --- |
| `error` | `[event: Event]` | Emitted when the avatar image fails to load |

## Slots (1)

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | — | Custom fallback content (overrides fallback prop) |

## Usage (from `packages/core/stories/media/DzAvatar.stories.ts`, story `Default`)

```vue
<DzAvatar v-bind="args" />
```

## Playground

Editable, running the **Size Gallery** story from `packages/core/stories/media/DzAvatar.stories.ts` (lines 126–140) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzAvatar" />

## Variants and controlled state

**Not declared.** `DzAvatar` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzAvatar` has no `DzAvatar.anatomy.ts`, so
...
**Not declared.** `DzAvatar` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzAvatar` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzAvatar` declares no anatomy, so there is no state union to show
examples for.

**Migration.** Breaking changes to this component are recorded in the repository's changesets
and published in the release notes; nothing is restated here, because a hand-typed migration
note drifts from the release it describes the first time the release changes. This component
last changed at `34d7ffd`.
...
| --- | --- | --- | --- |
| Props | 10 | 10 | 1 declare a default, of which 1 declare `undefined` (ADR-20 provider supplies the value) |
...
- **Component last changed at:** `34d7ffd3`
