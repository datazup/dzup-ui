...
- **Family:** Feedback
- **Source:** `packages/core/src/components/feedback/DzSkeleton.vue` · types `packages/core/src/components/feedback/DzSkeleton.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzSkeleton } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/feedback`
- **Risk tier:** A · **Status:** stable

## Intent and selection guidance

**Not declared.** `DzSkeleton` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `animate` | `boolean \| undefined` | no | `true` | Whether to animate the skeleton |
| `height` | `string \| undefined` | no | `undefined` | Custom height (CSS value) |
| `lines` | `number \| undefined` | no | `1` | Number of text lines to render (only for variant="text") |
| `variant` | `SkeletonVariant \| undefined` | no | `"text"` | Shape variant of the skeleton |
| `width` | `string \| undefined` | no | `undefined` | Custom width (CSS value) |

## Usage (from `packages/core/stories/feedback/DzSkeleton.stories.ts`, story `Default`)

```vue
<DzSkeleton v-bind="args" />
```

## Playground

Editable, running the **Variant Gallery** story from `packages/core/stories/feedback/DzSkeleton.stories.ts` (lines 82–103) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzSkeleton" />

## Variants and controlled state

**Not declared.** `DzSkeleton` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzSkeleton` has no `DzSkeleton.anatomy.ts`, so
...
**Not declared.** `DzSkeleton` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzSkeleton` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzSkeleton` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/feedback/DzSkeleton.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/feedback/DzSkeleton.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/feedback/DzSkeleton.stories.ts` |
