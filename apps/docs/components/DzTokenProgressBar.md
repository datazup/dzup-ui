---
title: "DzTokenProgressBar"
description: "Token-usage progress bar with threshold colors."
outline: [2, 3]
---
...
- **Family:** Feedback
- **Source:** `packages/core/src/components/feedback/DzTokenProgressBar.vue` · types `packages/core/src/components/feedback/DzTokenProgressBar.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzTokenProgressBar } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/feedback`
- **Risk tier:** A · **Status:** experimental

## Intent and selection guidance

**Not declared.** `DzTokenProgressBar` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `showWarning` | `boolean \| undefined` | no | `true` | Apply threshold colors at 70% (amber) and 90% (red). Default: true |
| `total` | `number` | yes | — | Token budget (denominator); must be > 0 |
...
| --- | --- | --- |
| `default` | `DzTokenProgressBarSlotProps` | Optional label text rendered next to the bar |

## Usage (from `packages/core/stories/_app-specific/DzTokenProgressBar.stories.ts`, story `Default`)

```vue
<div class="max-w-sm"><DzTokenProgressBar v-bind="args" /></div>
```

## Playground

Editable, running the **Threshold States** story from `packages/core/stories/_app-specific/DzTokenProgressBar.stories.ts` (lines 71–92) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzTokenProgressBar" />

## Variants and controlled state

**Not declared.** `DzTokenProgressBar` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzTokenProgressBar` has no `DzTokenProgressBar.anatomy.ts`, so
...
**Not declared.** `DzTokenProgressBar` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzTokenProgressBar` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/feedback/DzTokenProgressBar.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/feedback/DzTokenProgressBar.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/_app-specific/DzTokenProgressBar.stories.ts` |
