...
**Not declared.** `DzRadioGroup` declares no `@intent` block in its source header, so
...
| `modelValue` | `string \| undefined` | no | `""` | Value of the selected radio; the default empty string selects none. |
...
<DzRadioGroup v-model="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzRadioGroup :modelValue="value" @update:modelValue="value = $event" />
```

**Where each variant is shown.** 9 stories in
`packages/core/stories/forms/DzRadioGroup.stories.ts`: `Default`, `Horizontal`, `Size Gallery`, `Disabled`, `Dark Mode Preview`, `Interactive`, `Accessibility: Focus States`, `Real World: Payment Method`, `States`.
...
Declared in `packages/core/src/components/forms/DzRadioGroup.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/forms/DzRadioGroup.anatomy.ts`.
...
`DzRadioGroup` advertises 3 states:
`disabled`, `ready`, `required`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/forms/DzRadioGroup.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 11 | 11 | 5 declare a default, of which 6 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- | --- |
| `ArrowRight` | Move to and select the next radio. | `2.1.1` | [`radio-group`](https://www.w3.org/WAI/ARIA/apg/patterns/radio-group/) | swaps with the writing direction |
| `ArrowLeft` | Move to and select the previous radio. | `2.1.1` | [`radio-group`](https://www.w3.org/WAI/ARIA/apg/patterns/radio-group/) | swaps with the writing direction |
| `ArrowDown` | Move to and select the next radio. | `2.1.1` | [`radio-group`](https://www.w3.org/WAI/ARIA/apg/patterns/radio-group/) | — |
| `ArrowUp` | Move to and select the previous radio. | `2.1.1` | [`radio-group`](https://www.w3.org/WAI/ARIA/apg/patterns/radio-group/) | — |
| `Space` | Select the focused radio. | `2.1.1` | [`radio-group`](https://www.w3.org/WAI/ARIA/apg/patterns/radio-group/) | — |
...
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/forms/DzRadioGroup.spec.ts` — The component declares 6 binding(s); the unit spec asserts no key event for `ArrowRight`, `ArrowLeft`, `ArrowDown`, `ArrowUp`, `Space`, `Tab`. The contract is the yardstick, not the presence of any key at all. |
...
| `rtl-contract` | tier B | `present` | `packages/core/src/components/forms/DzRadioGroup.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
