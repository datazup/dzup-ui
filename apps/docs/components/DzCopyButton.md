---
title: "DzCopyButton"
description: "Clipboard copy button with visual feedback."
outline: [2, 3]
---
...
- **Family:** Buttons
- **Source:** `packages/core/src/components/buttons/DzCopyButton.vue` · types `packages/core/src/components/buttons/DzCopyButton.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzCopyButton } from '@dzup-ui/core'`
...
**Not declared.** `DzCopyButton` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `ariaLabel` | `string \| undefined` | no | `undefined` | Accessible label for the button |
| `copiedLabel` | `string \| undefined` | no | `undefined` | Label shown after a successful copy |
| `disabled` | `boolean \| undefined` | no | `false` | Disabled state -- prevents interaction |
| `id` | `string \| undefined` | no | `undefined` | Element ID |
| `label` | `string \| undefined` | no | `undefined` | Label shown alongside the copy icon |
| `size` | `CanonicalSize \| undefined` | no | `undefined` | Button size |
| `tone` | `CanonicalTone \| undefined` | no | `undefined` | Semantic color tone |
| `ui` | `Partial<Record<"root", DzClassValue>> \| undefined` | no | `undefined` | Per-part class overrides, keyed by the names in `DzCopyButton.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; `ui` addresses the other parts by name, and a typo is a type error. |
| `value` | `string` | yes | — | The value to copy to the clipboard |
| `variant` | `ButtonVariant \| undefined` | no | `undefined` | Visual style variant (fill / border treatment) |

## Events (1)

| Event | Payload | Description |
| --- | --- | --- |
| `copied` | `[value: string]` | Emitted when the value has been copied to clipboard |

## Slots (2)

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `{ copied: boolean; }` | Custom content replacing default label |
| `icon` | `{ copied: boolean; }` | Custom icon replacing default copy/check icons |

## Usage (verbatim story source from `packages/core/stories/buttons/DzCopyButton.stories.ts`, story `Default`)
...
Editable, running the **Size Gallery** story from `packages/core/stories/buttons/DzCopyButton.stories.ts` (lines 116–145) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzCopyButton" />
...
`packages/core/stories/buttons/DzCopyButton.stories.ts`: `Default`, `With Label`, `Size Gallery`, `Variant Gallery`, `Tone Gallery`, `States`, `Custom Icon + Label Slot`, `Dark Mode Preview`, `Accessibility: Announced State`, `Real World: Copy Install Command`, `Real World: API Key`, `Interactive: Copy Flow`.
...
Declared in `packages/core/src/components/buttons/DzCopyButton.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/buttons/DzCopyButton.anatomy.ts`.
...
`DzCopyButton` advertises 3 states:
`copied`, `disabled`, `idle`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/buttons/DzCopyButton.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 10 | 10 | 1 declare a default, of which 8 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `Enter` | Activate the control, copying the value to the clipboard. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
| `Space` | Activate the control, copying the value to the clipboard. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
...
Record: `e2e/at-matrix/DzCopyButton.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/buttons/DzCopyButton.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/buttons/DzCopyButton.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/buttons/DzCopyButton.stories.ts` |
| `ssr-sample` | tier A | **`unrun`** | — |
| `token-contrast` | tier A · corpus-wide | `pass` | `packages/tooling/src/token-checks/intent-text-contrast.ts` — Corpus gate: `yarn validate:tokens` covers every pair in the catalog at once. |
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/buttons/DzCopyButton.spec.ts` — The component declares 2 binding(s); the unit spec asserts no key event for `Enter`, `Space`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/buttons/DzCopyButton.stories.ts` |
| `controlled-uncontrolled` | tier B | **`unrun`** | The unit spec does not exercise both a controlled and an uncontrolled value path. |
| `browser-play` | tier B | `pass` | `packages/core/stories/buttons/DzCopyButton.stories.ts` |
| `rtl-contract` | tier B | `present` | `packages/core/src/components/buttons/DzCopyButton.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
...
| `at-manual` | tier B | **`unrun`** | `e2e/at-matrix/DzCopyButton.md` — 6 AT/browser pairs, none executed. |
