...
**Not declared.** `DzNumberInput` declares no `@intent` block in its source header, so
...
| `modelValue` | `number \| undefined` | no | `undefined` | The numeric value; `undefined` renders an empty field. |
...
<DzNumberInput v-model="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzNumberInput :modelValue="value" @update:modelValue="value = $event" />
```

**Where each variant is shown.** 14 stories in
`packages/core/stories/inputs/DzNumberInput.stories.ts`: `Default`, `Variant Gallery`, `Size Gallery`, `Min, Max, and Step`, `With Prefix Icon`, `States`, `Disabled`, `Invalid with Error Message`, `Dark Mode Preview`, `Interactive`, `Accessibility: Keyboard & ARIA`, `Real World: Quantity Selector`, `Real World: Settings Form`, `Readonly`.
...
| `decrement` | `[data-part="decrement"]` | yes |
| `error` | `[data-part="error"]` | no — renders zero or more than once |
| `increment` | `[data-part="increment"]` | yes |
| `input` | `[data-part="input"]` | yes |
| `prefix` | `[data-part="prefix"]` | no — renders zero or more than once |
| `root` | `[data-part="root"]` | yes |

```vue
<DzNumberInput :ui="{ 'control': 'ring-2', 'decrement': 'ring-2', 'error': 'ring-2', 'increment': 'ring-2', 'input': 'ring-2', 'prefix': 'ring-2', 'root': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/inputs/DzNumberInput.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/inputs/DzNumberInput.anatomy.ts`.
...
`DzNumberInput` advertises 4 states:
...
**Published examples:** `state-stories` is `pass` — `packages/core/stories/inputs/DzNumberInput.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 21 | 21 | 8 declare a default, of which 3 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `ArrowUp` | Increase the value by one step. | `2.1.1` | [`spinbutton`](https://www.w3.org/WAI/ARIA/apg/patterns/spinbutton/) |
| `ArrowDown` | Decrease the value by one step. | `2.1.1` | [`spinbutton`](https://www.w3.org/WAI/ARIA/apg/patterns/spinbutton/) |
...
| `keyboard-spec` | tier B | `present` | `packages/core/src/components/inputs/DzNumberInput.spec.ts` — All 2 declared binding(s) are exercised by the unit spec. |
