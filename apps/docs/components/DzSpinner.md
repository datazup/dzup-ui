...
Loading indicator with rotating animation.

- **Family:** Feedback
- **Source:** `packages/core/src/components/feedback/DzSpinner.vue` · types `packages/core/src/components/feedback/DzSpinner.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzSpinner } from '@dzup-ui/core'`
...
**Not declared.** `DzSpinner` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `label` | `string \| undefined` | no | `"Loading"` | Screen reader label |
| `size` | `CanonicalSize \| undefined` | no | `undefined` | Component size |
| `tone` | `CanonicalTone \| undefined` | no | `undefined` | Semantic color tone |

## Usage (from `packages/core/stories/feedback/DzSpinner.stories.ts`, story `Default`)

```vue
<DzSpinner v-bind="args" />
```

## Playground

Editable, running the **Size Gallery** story from `packages/core/stories/feedback/DzSpinner.stories.ts` (lines 74–87) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzSpinner" />
...
`packages/core/stories/feedback/DzSpinner.stories.ts`: `Default`, `Size Gallery`, `Tone Gallery`, `With Custom Label`, `Interactive`, `Accessibility: Screen Reader Labels`, `Dark Mode Preview`, `Real World: Inline Loading`.
...
Declared in `packages/core/src/components/feedback/DzSpinner.anatomy.ts`.
...
`DzSpinner` advertises 1 state:
`loading`. Each is emitted as `data-state` or as a
...
| --- | --- | --- | --- |
| Props | 3 | 3 | 1 declare a default, of which 2 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/feedback/DzSpinner.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/feedback/DzSpinner.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/feedback.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/feedback/DzSpinner.stories.ts` |
