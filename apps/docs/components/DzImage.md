---
title: "DzImage"
description: "Enhanced image component with loading and error states."
outline: [2, 3]
---
...
- **Family:** Media
- **Source:** `packages/core/src/components/media/DzImage.vue` · types `packages/core/src/components/media/DzImage.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzImage } from '@dzup-ui/core'`
...
| --- | --- | --- |
| `error` | `[event: Event]` | Image failed to load |
| `load` | `[event: Event]` | Image loaded successfully |

## Slots (2)

| Slot | Slot props | Description |
| --- | --- | --- |
| `error` | — | Custom error/fallback content |
| `loading` | — | Custom loading placeholder |

## Usage (from `packages/core/stories/media/DzImage.stories.ts`, story `Default`)

```vue
<div class="w-80"><DzImage v-bind="args" /></div>
```

## Playground

Editable, running the **Object Fit Gallery** story from `packages/core/stories/media/DzImage.stories.ts` (lines 112–132) **verbatim**. Edits run in a sandbox; nothing here is saved.
...
| --- | --- | --- | --- |
| Props | 11 | 11 | 2 declare a default |
...
- **Security boundary:** `url` — The `src` prop is a host-supplied URL that becomes a subresource load; the error state is what a blocked or hostile URL surfaces as.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/media/DzImage.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/media/DzImage.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/media.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/media/DzImage.stories.ts` |
