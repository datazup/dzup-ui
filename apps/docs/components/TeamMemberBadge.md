---
title: "TeamMemberBadge"
description: "Team participant status pill."
outline: [2, 3]
---
...
Team participant status pill.

- **Family:** Feedback
- **Source:** `packages/core/src/components/feedback/TeamMemberBadge.vue` · types `packages/core/src/components/feedback/TeamMemberBadge.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { TeamMemberBadge } from '@dzup-ui/core'`
...
**Not declared.** `TeamMemberBadge` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `participantId` | `string` | yes | — | Participant ID within the team run (not the team-definition ID). |
| `role` | `string` | yes | — | Participant role label (e.g. "planner", "executor"). |
| `size` | `CanonicalSize \| undefined` | no | `"md"` | Badge size |
| `status` | `TeamMemberStatus` | yes | — | Live participant status from the team runtime. |

## Slots (1)

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `{ role: string; status: TeamMemberStatus; }` | Override the default label (defaults to role text). Receives role + status as slot props. |

## Usage (from `packages/core/stories/_app-specific/TeamMemberBadge.stories.ts`, story `Default`)

```vue
<TeamMemberBadge v-bind="args" />
```

## Playground

Editable, running the **Status Gallery** story from `packages/core/stories/_app-specific/TeamMemberBadge.stories.ts` (lines 78–97) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="TeamMemberBadge" />

## Variants and controlled state

**Not declared.** `TeamMemberBadge` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `TeamMemberBadge` has no `DzTeamMemberBadge.anatomy.ts`, so
...
**Not declared.** `TeamMemberBadge` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `TeamMemberBadge` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `TeamMemberBadge` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/feedback/TeamMemberBadge.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/feedback/TeamMemberBadge.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/_app-specific/TeamMemberBadge.stories.ts` |
