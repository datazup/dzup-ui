...
- **Anatomy parts (ADR-19):** `item-label`, `root`

## Intent and selection guidance

**Not declared.** `DzListItem` declares no `@intent` block in its source header, so
...
| `ui` | `Partial<Record<"root" \| "item-label", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzListItem.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; a key outside the declared parts is a type error, not a class that lands nowhere. |

## Events (1)

| Event | Payload | Description |
| --- | --- | --- |
| `click` | `[event: MouseEvent]` | Emitted when an interactive item is clicked |
...
`packages/core/stories/data/DzListItem.stories.ts`: `Default`, `All Tones`, `Active State`, `With Prefix & Suffix Slots`, `Disabled`, `Dark Mode Preview`.
...
| --- | --- | --- |
| `item-label` | `[data-part="item-label"]` | yes |
| `root` | `[data-part="root"]` | yes |

```vue
<DzListItem :ui="{ 'item-label': 'ring-2', 'root': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/data/DzListItem.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/data/DzListItem.anatomy.ts`.
...
`DzListItem` advertises 2 states:
...
| --- | --- | --- | --- |
| Props | 9 | 9 | 2 declare a default, of which 1 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- | --- |
| `Enter` | `interactive` | Activate the row. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
| `Space` | `interactive` | Activate the row. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
...
**6 unrun:** `unit-spec`, `ssr-sample`, `keyboard-spec`, `controlled-uncontrolled`, `browser-matrix`, `at-manual`. They are named rather than counted, because a total tells a reader nothing about what is missing.
