---
title: "DzCaption"
description: "Small caption text component."
outline: [2, 3]
---
...
- **Family:** Typography
- **Source:** `packages/core/src/components/typography/DzCaption.vue` · types `packages/core/src/components/typography/DzCaption.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzCaption } from '@dzup-ui/core'`
...
**Not declared.** `DzCaption` declares no `@intent` block in its source header, so
...
| `tone` | `CaptionTone \| undefined` | no | `"muted"` | Semantic color tone |
| `ui` | `Partial<Record<"root", DzClassValue>> \| undefined` | no | `undefined` | Per-part class overrides, keyed by the names in `DzCaption.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; `ui` addresses the other parts by name, and a typo is a type error. |

## Slots (1)

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | — | Caption text content |

## Usage (from `packages/core/stories/typography/DzCaption.stories.ts`, story `Default`)

```vue
<DzCaption v-bind="args">Last updated: 2 hours ago</DzCaption>
```

## Playground

Editable, running the **Tone Gallery** story from `packages/core/stories/typography/DzCaption.stories.ts` (lines 66–84) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzCaption" />
...
`packages/core/stories/typography/DzCaption.stories.ts`: `Default`, `Tone Gallery`, `With Form Fields`, `As Metadata`, `Dark Mode Preview`, `Accessibility: ARIA Integration`, `Real World: Image Caption`, `Real World: Validation Messages`.
...
Declared in `packages/core/src/components/typography/DzCaption.anatomy.ts`.
...
| --- | --- | --- | --- |
| Props | 3 | 3 | 1 declare a default, of which 1 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/typography/DzCaption.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/typography/DzCaption.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/typography.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/typography/DzCaption.stories.ts` |
