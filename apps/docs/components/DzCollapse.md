---
title: "DzCollapse"
description: "Animated expand/collapse container."
outline: [2, 3]
---
...
- **Family:** Layout
- **Source:** `packages/core/src/components/layout/DzCollapse.vue` · types `packages/core/src/components/layout/DzCollapse.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzCollapse } from '@dzup-ui/core'`
...
**Not declared.** `DzCollapse` declares no `@intent` block in its source header, so
...
| `duration` | `number \| undefined` | no | `200` | Animation duration in milliseconds |
| `id` | `string \| undefined` | no | — | Unique element ID (prefer `useId()` from Vue 3.5 when auto-generated) |
| `modelValue` | `boolean \| undefined` | no | `false` | Whether the panel is expanded; `false` renders it collapsed. |
...
## Usage (from `packages/core/stories/layout/DzCollapse.stories.ts`, story `Default`)
...
Editable, running the **Duration Gallery** story from `packages/core/stories/layout/DzCollapse.stories.ts` (lines 96–127) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzCollapse" />
...
<DzCollapse v-model="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzCollapse :modelValue="value" @update:modelValue="value = $event" />
```

**Where each variant is shown.** 9 stories in
`packages/core/stories/layout/DzCollapse.stories.ts`: `Default`, `Duration Gallery`, `Initially Collapsed`, `Multiple Sections`, `Dark Mode Preview`, `Interactive: Toggle Counter`, `Real World: FAQ Section`, `Real World: Filter Panel`, `Accessibility: ARIA States`.
...
Declared in `packages/core/src/components/layout/DzCollapse.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/layout/DzCollapse.anatomy.ts`.
...
`DzCollapse` advertises 2 states:
...
**Published examples:** `state-stories` is `pass` — `packages/core/stories/layout/DzCollapse.stories.ts`.
...
| --- | --- | --- | --- | --- |
| `Enter` | `trigger` | Expand or collapse the region. | `2.1.1` | [`disclosure`](https://www.w3.org/WAI/ARIA/apg/patterns/disclosure/) |
| `Space` | `trigger` | Expand or collapse the region. | `2.1.1` | [`disclosure`](https://www.w3.org/WAI/ARIA/apg/patterns/disclosure/) |
...
Record: `e2e/at-matrix/DzCollapse.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/layout/DzCollapse.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/layout/DzCollapse.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/layout.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/layout/DzCollapse.stories.ts` |
| `ssr-sample` | tier A | **`unrun`** | — |
| `token-contrast` | tier A · corpus-wide | `pass` | `packages/tooling/src/token-checks/intent-text-contrast.ts` — Corpus gate: `yarn validate:tokens` covers every pair in the catalog at once. |
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/layout/DzCollapse.spec.ts` — The component declares 2 binding(s); the unit spec asserts no key event for `Enter`, `Space`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/layout/DzCollapse.stories.ts` |
| `controlled-uncontrolled` | tier B | **`unrun`** | The unit spec does not exercise both a controlled and an uncontrolled value path. |
| `browser-play` | tier B | `pass` | `packages/core/stories/layout/DzCollapse.stories.ts` |
| `rtl-contract` | tier B | `present` | `packages/core/src/components/layout/DzCollapse.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
...
| `at-manual` | tier B | **`unrun`** | `e2e/at-matrix/DzCollapse.md` — 6 AT/browser pairs, none executed. |
