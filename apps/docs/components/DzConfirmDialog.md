...
- **Anatomy parts (ADR-19):** `action`, `content`, `description`, `icon`, `overlay`, `title`, `viewport`

## Intent and selection guidance

**Not declared.** `DzConfirmDialog` declares no `@intent` block in its source header, so
...
## Props (14, of which 3 inherited from `@dzup-ui/contracts`)
...
| `ui` | `DzConfirmDialogUi \| undefined` | no | — | Per-part class overrides for the four nodes this component renders inside the dialog, keyed by the names in `DzConfirmDialog.anatomy.ts` (ADR-19 §5). The overlay, content and viewport belong to `DzDialogContent` and are reached through `overlayClass` and `class`. |
...
<DzConfirmDialog v-model:open="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzConfirmDialog :open="value" @update:open="value = $event" />
```

**Where each variant is shown.** 10 stories in
`packages/core/stories/overlays/DzConfirmDialog.stories.ts`: `Default`, `Variant Gallery`, `Danger: Destructive Confirm`, `States: Async Confirm (Loading)`, `With Custom Slot Content`, `Dark Mode Preview`, `Accessibility: Labelled Dialog`, `Real World: Unsaved Changes Guard`, `Interactive: Confirm Flow`, `States`.
...
```vue
<DzConfirmDialog :ui="{ 'action': 'ring-2', 'content': 'ring-2', 'description': 'ring-2', 'icon': 'ring-2', 'overlay': 'ring-2', 'title': 'ring-2', 'viewport': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/overlays/DzConfirmDialog.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/overlays/DzConfirmDialog.anatomy.ts`.
...
`DzConfirmDialog` advertises 2 states:
...
**Published examples:** `state-stories` is `pass` — `packages/core/stories/overlays/DzConfirmDialog.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 14 | 14 | 6 declare a default, of which 6 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `Escape` | Dismiss the dialog without taking the action. | `2.1.1`, `2.1.2` | [`alertdialog`](https://www.w3.org/WAI/ARIA/apg/patterns/alertdialog/) |
| `Tab` | Move between the actions, wrapping inside the dialog. | `2.1.2` | [`alertdialog`](https://www.w3.org/WAI/ARIA/apg/patterns/alertdialog/) |
...
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/overlays/DzConfirmDialog.spec.ts` — The component declares 3 binding(s); the unit spec asserts no key event for `Escape`, `Tab`. The contract is the yardstick, not the presence of any key at all. |
...
| `rtl-contract` | tier B | `present` | `packages/core/src/components/overlays/DzConfirmDialog.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
...
**6 unrun:** `axe`, `ssr-sample`, `keyboard-spec`, `controlled-uncontrolled`, `portal-hydration`, `at-manual`. They are named rather than counted, because a total tells a reader nothing about what is missing.
