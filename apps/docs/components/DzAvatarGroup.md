---
title: "DzAvatarGroup"
description: "Stacked avatar display with overflow count."
outline: [2, 3]
---
...
Stacked avatar display with overflow count.

- **Family:** Media
- **Source:** `packages/core/src/components/media/DzAvatarGroup.vue` · types `packages/core/src/components/media/DzAvatarGroup.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzAvatarGroup } from '@dzup-ui/core'`
...
**Not declared.** `DzAvatarGroup` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `ariaLabel` | `string \| undefined` | no | — | Accessible label for the group |
| `id` | `string \| undefined` | no | — | Unique element ID |
| `max` | `number \| undefined` | no | — | Maximum number of avatars to display before showing overflow count |
| `size` | `CanonicalSize \| undefined` | no | `"md"` | Size propagated to all child avatars |

## Slots (1)

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | — | DzAvatar children |

## Usage (from `packages/core/stories/media/DzAvatarGroup.stories.ts`, story `Default`)
...
Editable, running the **Size Gallery** story from `packages/core/stories/media/DzAvatarGroup.stories.ts` (lines 99–116) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzAvatarGroup" />

## Variants and controlled state

**Not declared.** `DzAvatarGroup` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzAvatarGroup` has no `DzAvatarGroup.anatomy.ts`, so
...
**Not declared.** `DzAvatarGroup` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzAvatarGroup` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzAvatarGroup` declares no anatomy, so there is no state union to show
examples for.
...
- **Traits:** `dataset`
- **Security boundary:** `url` — Each avatar carries a host-supplied `src`.
...
**2 unrun:** `ssr-sample`, `data-scenarios`. They are named rather than counted, because a total tells a reader nothing about what is missing.
