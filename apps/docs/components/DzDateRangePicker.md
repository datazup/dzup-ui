---
title: "DzDateRangePicker"
description: "Date range selection using Reka UI (ADR-07)."
outline: [2, 3]
---
...
Date range selection using Reka UI (ADR-07).

- **Family:** Forms
- **Source:** `packages/core/src/components/forms/DzDateRangePicker.vue` · types `packages/core/src/components/forms/DzDateRangePicker.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzDateRangePicker } from '@dzup-ui/core'`
...
| `change` | `[value: DateRangeValue, metadata?: ChangeMetadata \| undefined]` | Value committed (after user finishes editing, not during typing) |
...
| --- | --- | --- |
| `trigger` | `{ value: DateRangeValue \| undefined; placeholder: string \| undefined; }` | Custom trigger content |

## Usage (from `packages/core/stories/forms/DzDateRangePicker.stories.ts`, story `Default`)

```vue
<DzDateRangePicker v-bind="args" class="max-w-sm" />
```

## Playground

Editable, running the **Variant Gallery** story from `packages/core/stories/forms/DzDateRangePicker.stories.ts` (lines 126–138) **verbatim**. Edits run in a sandbox; nothing here is saved.
...
Script: `e2e/at-matrix/scripts/DzDateRangePicker.at-script.md` · record: `e2e/at-matrix/DzDateRangePicker.md` (append-only).
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/forms/DzDateRangePicker.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/forms/DzDateRangePicker.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
