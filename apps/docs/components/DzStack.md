...
- **Family:** Layout
- **Source:** `packages/core/src/components/layout/DzStack.vue` · types `packages/core/src/components/layout/DzStack.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzStack } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/layout`
- **Risk tier:** A · **Status:** stable

## Intent and selection guidance

**Not declared.** `DzStack` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `align` | `StackAlign \| undefined` | no | `undefined` | Align items along the cross axis |
...
## Usage (from `packages/core/stories/layout/DzStack.stories.ts`, story `Default`)
...
Editable, running the **Direction Gallery** story from `packages/core/stories/layout/DzStack.stories.ts` (lines 107–132) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzStack" />

## Variants and controlled state

**Not declared.** `DzStack` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzStack` has no `DzStack.anatomy.ts`, so
...
**Not declared.** `DzStack` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzStack` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzStack` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/layout/DzStack.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/layout/DzStack.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/layout.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/layout/DzStack.stories.ts` |
