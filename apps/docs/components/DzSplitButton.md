---
title: "DzSplitButton"
description: "Compound split button with primary action + dropdown."
outline: [2, 3]
---
...
Compound split button with primary action + dropdown.

- **Family:** Buttons
- **Source:** `packages/core/src/components/buttons/DzSplitButton.vue` · types `packages/core/src/components/buttons/DzSplitButton.types.ts`
- **Compound parts:** `DzSplitButtonAction`, `DzSplitButtonMenu`
- **Install:** `npm i @dzup-ui/core` — then `import { DzSplitButton } from '@dzup-ui/core'`
...
- **Anatomy parts (ADR-19):** `action`, `root`, `trigger`

## Intent and selection guidance

**Not declared.** `DzSplitButton` declares no `@intent` block in its source header, so
...
| `loading` | `boolean \| undefined` | no | `false` | Loading state -- shows spinner on primary action |
| `size` | `CanonicalSize \| undefined` | no | `undefined` | Component size |
| `tone` | `CanonicalTone \| undefined` | no | `undefined` | Semantic color tone |
| `ui` | `DzSplitButtonUi \| undefined` | no | `undefined` | Per-part class overrides, keyed by the names in `DzSplitButton.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; `ui` addresses the other parts by name, and a typo is a type error. |
...
| --- | --- | --- |
| `default` | — | DzSplitButtonAction and DzSplitButtonMenu children |

## Usage (from `packages/core/stories/buttons/DzSplitButton.stories.ts`, story `Default`)
...
Editable, running the **Variant Gallery** story from `packages/core/stories/buttons/DzSplitButton.stories.ts` (lines 116–145) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzSplitButton" />

## Compound parts

`DzSplitButton` is used with 2 sub-parts, documented here rather than on pages of their own because they are not usable outside it.

### DzSplitButtonAction

Primary action button within DzSplitButton.

- **Install:** `npm i @dzup-ui/core` — then `import { DzSplitButtonAction } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/buttons`
- **Compound part of:** `DzSplitButton`

#### Props (1)

| Prop | Type | Required | Declared default | Description |
| --- | --- | --- | --- | --- |
| `ariaLabel` | `string \| undefined` | no | — | Accessible label for the primary action |
...
A compound sub-part of `DzSplitButton`; see that component's usage snippet.

### DzSplitButtonMenu

Dropdown trigger within DzSplitButton.

- **Install:** `npm i @dzup-ui/core` — then `import { DzSplitButtonMenu } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/buttons`
- **Compound part of:** `DzSplitButton`
...
| Prop | Type | Required | Declared default | Description |
| --- | --- | --- | --- | --- |
| `ariaLabel` | `string \| undefined` | no | — | Accessible label for the dropdown trigger |
...
`packages/core/stories/buttons/DzSplitButton.stories.ts`: `Default`, `Variant Gallery`, `Size Gallery`, `Tone Gallery`, `Disabled`, `Loading`, `States`, `Dark Mode Preview`, `Interactive`, `Interactive: Menu Select`, `Accessibility: Focus States`, `Real World: Save with Options`.
...
```vue
<DzSplitButton :ui="{ 'action': 'ring-2', 'root': 'ring-2', 'trigger': 'ring-2' }" />
```
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/buttons/DzSplitButton.anatomy.ts`.
...
`DzSplitButton` advertises 3 states:
...
**Published examples:** `state-stories` is `pass` — `packages/core/stories/buttons/DzSplitButton.stories.ts`.
...
Tasks this component owes on each pair: `reach`, `open`, `navigate`, `activate`, `dismiss`.
...
Record: `e2e/at-matrix/DzSplitButton.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/buttons/DzSplitButton.contract.spec.ts` |
