---
title: "DzRadio"
description: "Radio option using Reka UI (ADR-07)."
outline: [2, 3]
---
...
Radio option using Reka UI (ADR-07).

- **Family:** Forms
- **Source:** `packages/core/src/components/forms/DzRadio.vue` · types `packages/core/src/components/forms/DzRadio.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzRadio } from '@dzup-ui/core'`
...
**Not declared.** `DzRadio` declares no `@intent` block in its source header, so
...
| `ui` | `Partial<Record<"root" \| "indicator" \| "control" \| "label", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzRadio.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target. |
| `value` | `string` | yes | — | The value of this radio option (required) |
...
## Usage (from `packages/core/stories/forms/DzRadio.stories.ts`, story `Default`)
...
Editable, running the **Size Gallery** story from `packages/core/stories/forms/DzRadio.stories.ts` (lines 79–95) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzRadio" />
...
`packages/core/stories/forms/DzRadio.stories.ts`: `Default`, `Size Gallery`, `Disabled`, `States`, `Dark Mode Preview`, `Accessibility: Focus States`.
...
```vue
<DzRadio :ui="{ 'control': 'ring-2', 'indicator': 'ring-2', 'label': 'ring-2', 'root': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/forms/DzRadio.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/forms/DzRadio.anatomy.ts`.
...
`DzRadio` advertises 6 states:
`active`, `checked`, `disabled`, `idle`, `invalid`, `unchecked`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/forms/DzRadio.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 9 | 9 | 2 declare a default, of which 5 declare `undefined` (ADR-20 provider supplies the value) |
...
Record: `e2e/at-matrix/DzRadio.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/forms/DzRadio.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/forms/DzRadio.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/forms.a11y.spec.ts` · `packages/core/tests/a11y/inputs.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/forms/DzRadio.stories.ts` |
| `ssr-sample` | tier A | `present` | `packages/core/tests/ssr/ssr-smoke.spec.ts` |
| `token-contrast` | tier A · corpus-wide | `pass` | `packages/tooling/src/token-checks/intent-text-contrast.ts` — Corpus gate: `yarn validate:tokens` covers every pair in the catalog at once. |
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/forms/DzRadio.spec.ts` — The component declares 1 binding(s); the unit spec asserts no key event for `Space`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/forms/DzRadio.stories.ts` |
| `controlled-uncontrolled` | tier B | **`unrun`** | The unit spec does not exercise both a controlled and an uncontrolled value path. |
| `browser-play` | tier B | `pass` | `packages/core/stories/forms/DzRadio.stories.ts` |
| `rtl-contract` | tier B | `present` | `packages/core/src/components/forms/DzRadio.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
...
| `at-manual` | tier B | **`unrun`** | `e2e/at-matrix/DzRadio.md` — 6 AT/browser pairs, none executed. |
