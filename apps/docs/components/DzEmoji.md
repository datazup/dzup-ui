---
title: "DzEmoji"
description: "Accessible emoji renderer with consistent sizing."
outline: [2, 3]
---
...
Accessible emoji renderer with consistent sizing.

- **Family:** Media
- **Source:** `packages/core/src/components/media/DzEmoji.vue` · types `packages/core/src/components/media/DzEmoji.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzEmoji } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/media`
- **Risk tier:** A · **Status:** stable

## Intent and selection guidance

**Not declared.** `DzEmoji` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `emoji` | `string` | yes | — | The emoji character(s) to render, e.g. `'🎉'` or `'👨‍👩‍👧‍👦'` |
| `id` | `string \| undefined` | no | — | Accessible identifier |
| `label` | `string \| undefined` | no | — | Accessible label. When provided, the emoji is treated as meaningful (`role="img"`, `aria-label`). When omitted, it is decorative (`aria-hidden="true"`) — use this next to visible text that already conveys the meaning. |
| `size` | `EmojiSize \| undefined` | no | `"md"` | Emoji size (font-size scale) |

## Usage (verbatim story source from `packages/core/stories/media/DzEmoji.stories.ts`, story `Default`)
...
Editable, running the **Size Gallery** story from `packages/core/stories/media/DzEmoji.stories.ts` (lines 68–97) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzEmoji" />

## Variants and controlled state

**Not declared.** `DzEmoji` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzEmoji` has no `DzEmoji.anatomy.ts`, so
...
**Not declared.** `DzEmoji` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzEmoji` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzEmoji` declares no anatomy, so there is no state union to show
examples for.

**Migration.** Breaking changes to this component are recorded in the repository's changesets
and published in the release notes; nothing is restated here, because a hand-typed migration
note drifts from the release it describes the first time the release changes. This component
last changed at `ca9c390`.
...
- **Component last changed at:** `ca9c3906`
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/media/DzEmoji.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/media/DzEmoji.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/media/DzEmoji.stories.ts` |
