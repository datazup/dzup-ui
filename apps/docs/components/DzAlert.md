---
title: "DzAlert"
description: "Contextual alert messages for user feedback."
outline: [2, 3]
---
...
Contextual alert messages for user feedback.

- **Family:** Feedback
- **Source:** `packages/core/src/components/feedback/DzAlert.vue` · types `packages/core/src/components/feedback/DzAlert.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzAlert } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/feedback`
- **Risk tier:** A · **Status:** stable
- **Taxonomy:** variant: `filled` `outline` `subtle` `ghost` · tone: `neutral` `primary` `success` `warning` `danger` `info`

## Intent and selection guidance

**Not declared.** `DzAlert` declares no `@intent` block in its source header, so
...
| `closable` | `boolean \| undefined` | no | `false` | Whether the alert can be dismissed |
| `icon` | `Component \| undefined` | no | `undefined` | Optional icon component to display |
| `id` | `string \| undefined` | no | — | Unique element ID (prefer `useId()` from Vue 3.5 when auto-generated) |
| `title` | `string \| undefined` | no | `undefined` | Alert title text |
| `tone` | `CanonicalTone \| undefined` | no | `"info"` | Semantic color tone |
| `variant` | `AlertVariant \| undefined` | no | `"subtle"` | Visual style variant |
...
## Usage (from `packages/core/stories/feedback/DzAlert.stories.ts`, story `Default`)

```vue
<DzAlert v-bind="args">This is an informational alert message.</DzAlert>
```

## Playground

Editable, running the **Variant Gallery** story from `packages/core/stories/feedback/DzAlert.stories.ts` (lines 97–110) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzAlert" />

## Variants and controlled state

**Not declared.** `DzAlert` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzAlert` has no `DzAlert.anatomy.ts`, so
...
**Not declared.** `DzAlert` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzAlert` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| Props | 10 | 10 | 3 declare a default, of which 2 declare `undefined` (ADR-20 provider supplies the value) |
