...
**Not declared.** `DzAnchor` declares no `@intent` block in its source header, so
...
| `ui` | `Partial<Record<"root" \| "item" \| "list", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzAnchor.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; every other node in the rail is rendered by this component's own render function, so `ui` is the only way to reach it. |
...
<DzAnchor v-model:active="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzAnchor :active="value" @update:active="value = $event" />
```

**Where each variant is shown.** 6 stories in
`packages/core/stories/navigation/DzAnchor.stories.ts`: `Default`, `Nested sections`, `Affixed + offsetTop`, `Controlled (v-model:active)`, `Dark Mode Preview`, `States`.
...
```vue
<DzAnchor :ui="{ 'item': 'ring-2', 'list': 'ring-2', 'root': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/navigation/DzAnchor.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/navigation/DzAnchor.anatomy.ts`.
...
`DzAnchor` advertises 3 states:
`active`, `ready`, `url-rejected`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/navigation/DzAnchor.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 9 | 9 | 3 declare a default, of which 4 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `Enter` | Follow the link. | `2.1.1` | [`link`](https://www.w3.org/WAI/ARIA/apg/patterns/link/) |
...
- **Measured:** `keyboard-spec` is **unrun** — The component declares 1 binding(s); the unit spec asserts no key event for `Enter`. The contract is the yardstick, not the presence of any key at all.
...
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/navigation/DzAnchor.spec.ts` — The component declares 1 binding(s); the unit spec asserts no key event for `Enter`. The contract is the yardstick, not the presence of any key at all. |
...
| `rtl-contract` | tier B | `present` | `packages/core/src/components/navigation/DzAnchor.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
...
**6 unrun:** `axe`, `ssr-sample`, `keyboard-spec`, `controlled-uncontrolled`, `data-scenarios`, `at-manual` · **1 stale:** `browser-matrix`. They are named rather than counted, because a total tells a reader nothing about what is missing.
