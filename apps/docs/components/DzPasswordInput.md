...
- **Family:** Inputs
- **Source:** `packages/core/src/components/inputs/DzPasswordInput.vue` · types `packages/core/src/components/inputs/DzPasswordInput.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzPasswordInput } from '@dzup-ui/core'`
...
- **Anatomy parts (ADR-19):** `control`, `error`, `input`, `prefix`, `root`, `spinner`, `toggle`

## Intent and selection guidance

**Not declared.** `DzPasswordInput` declares no `@intent` block in its source header, so
...
| `modelValue` | `string \| undefined` | no | `""` | The password text; the default empty string renders an empty field. |
...
| --- | --- | --- |
| `prefix` | — | Content rendered before the input (icon) |
...
## Usage (from `packages/core/stories/inputs/DzPasswordInput.stories.ts`, story `Default`)

```vue
<DzPasswordInput v-bind="args" class="max-w-xs" />
```

## Playground

Editable, running the **Variant Gallery** story from `packages/core/stories/inputs/DzPasswordInput.stories.ts` (lines 141–153) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzPasswordInput" />
...
<DzPasswordInput v-model="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzPasswordInput :modelValue="value" @update:modelValue="value = $event" />
```

**Where each variant is shown.** 13 stories in
`packages/core/stories/inputs/DzPasswordInput.stories.ts`: `Default`, `Variant Gallery`, `Size Gallery`, `With Prefix Icon`, `States`, `Disabled`, `Invalid with Error Message`, `Dark Mode Preview`, `Interactive`, `Accessibility: Toggle & ARIA`, `Real World: Change Password`, `Real World: Sign Up Form`, `Readonly`.
...
| `toggle` | `[data-part="toggle"]` | yes |

```vue
<DzPasswordInput :ui="{ 'control': 'ring-2', 'error': 'ring-2', 'input': 'ring-2', 'prefix': 'ring-2', 'root': 'ring-2', 'spinner': 'ring-2', 'toggle': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/inputs/DzPasswordInput.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/inputs/DzPasswordInput.anatomy.ts`.
...
`DzPasswordInput` advertises 4 states:
...
**Published examples:** `state-stories` is `pass` — `packages/core/stories/inputs/DzPasswordInput.stories.ts`.
...
| --- | --- | --- | --- | --- |
| `Enter` | `toggle` | Show or hide the password. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
| `Space` | `toggle` | Show or hide the password. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
...
Record: `e2e/at-matrix/DzPasswordInput.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/inputs/DzPasswordInput.spec.ts` — The component declares 2 binding(s); the unit spec asserts no key event for `Enter`, `Space`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/inputs/DzPasswordInput.stories.ts` |
