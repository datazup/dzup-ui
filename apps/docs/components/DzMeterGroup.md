---
title: "DzMeterGroup"
description: "Segmented proportional meter with legend."
outline: [2, 3]
---
...
Segmented proportional meter with legend.

- **Family:** Feedback
- **Source:** `packages/core/src/components/feedback/DzMeterGroup.vue` · types `packages/core/src/components/feedback/DzMeterGroup.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzMeterGroup } from '@dzup-ui/core'`
...
**Not declared.** `DzMeterGroup` declares no `@intent` block in its source header, so
...
| `max` | `number \| undefined` | no | — | Maximum value of the whole. Defaults to the sum of all segment values. |
| `orientation` | `Orientation \| undefined` | no | `"horizontal"` | Track orientation |
| `showLegend` | `boolean \| undefined` | no | `true` | Whether to render the legend |
| `size` | `CanonicalSize \| undefined` | no | `undefined` | Component size (controls track thickness) |
| `values` | `DzMeterGroupSegment[] \| undefined` | no | `[]` | Ordered list of segments to render |

## Slots (4)

| Slot | Slot props | Description |
| --- | --- | --- |
| `end` | `DzMeterGroupSlotProps` | Content rendered after the meter track |
| `label` | `DzMeterGroupSlotProps` | Heading/label content rendered above the meter |
| `legend` | `DzMeterGroupSlotProps` | Custom legend; defaults to chips with label + value |
| `start` | `DzMeterGroupSlotProps` | Content rendered before the meter track |

## Usage (from `packages/core/stories/feedback/DzMeterGroup.stories.ts`, story `Default`)

```vue
<DzMeterGroup v-bind="args" class="max-w-md" />
```

## Playground

Editable, running the **Storage Breakdown** story from `packages/core/stories/feedback/DzMeterGroup.stories.ts` (lines 114–141) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzMeterGroup" />

## Variants and controlled state

**Not declared.** `DzMeterGroup` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzMeterGroup` has no `DzMeterGroup.anatomy.ts`, so
...
**Not declared.** `DzMeterGroup` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzMeterGroup` declares no anatomy, so there is no state union to show
examples for.
...
- **APG pattern:** [`meter`](https://www.w3.org/WAI/ARIA/apg/patterns/meter/)
...
- **Pattern:** [APG — `meter`](https://www.w3.org/WAI/ARIA/apg/patterns/meter/) · its *Keyboard Interaction* section is
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/feedback/DzMeterGroup.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/feedback/DzMeterGroup.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/feedback/DzMeterGroup.stories.ts` |
