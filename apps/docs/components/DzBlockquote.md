---
title: "DzBlockquote"
description: "Styled blockquote component."
outline: [2, 3]
---
...
- **Family:** Typography
- **Source:** `packages/core/src/components/typography/DzBlockquote.vue` · types `packages/core/src/components/typography/DzBlockquote.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzBlockquote } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/typography`
- **Risk tier:** A · **Status:** stable
- **Anatomy parts (ADR-19):** `content`, `footer`, `root`

## Intent and selection guidance

**Not declared.** `DzBlockquote` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `cite` | `string \| undefined` | no | — | Attribution source or author |
| `id` | `string \| undefined` | no | — | Unique element ID |
| `ui` | `Partial<Record<"root" \| "content" \| "footer", DzClassValue>> \| undefined` | no | `undefined` | Per-part class overrides, keyed by the names in `DzBlockquote.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; `ui` addresses the other parts by name, and a typo is a type error. |
...
## Usage (from `packages/core/stories/typography/DzBlockquote.stories.ts`, story `Default`)
...
Editable, running the **With Attribution** story from `packages/core/stories/typography/DzBlockquote.stories.ts` (lines 65–78) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzBlockquote" />

## Variants and controlled state

**Not declared.** `DzBlockquote` declares no recipe axes in its anatomy and exposes no
...
```vue
<DzBlockquote :ui="{ 'content': 'ring-2', 'footer': 'ring-2', 'root': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/typography/DzBlockquote.anatomy.ts`.
...
| --- | --- | --- | --- |
| Props | 3 | 3 | 0 declare a default, of which 1 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/typography/DzBlockquote.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/typography/DzBlockquote.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/typography.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/typography/DzBlockquote.stories.ts` |
