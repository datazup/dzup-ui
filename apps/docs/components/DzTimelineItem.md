---
title: "DzTimelineItem"
description: "Child item within a DzTimeline compound component."
outline: [2, 3]
---
...
Child item within a DzTimeline compound component.

- **Family:** Data display
- **Source:** `packages/core/src/components/data/DzTimelineItem.vue`
- **Install:** `npm i @dzup-ui/core` — then `import { DzTimelineItem } from '@dzup-ui/core'`
...
**Not declared.** `DzTimelineItem` declares no `@intent` block in its source header, so
...
| `status` | `string \| undefined` | no | `undefined` | Status text (e.g., time, date) |
| `tone` | `CanonicalTone \| undefined` | no | `"neutral"` | Semantic color tone for the item indicator |
...
| `indicator` | — | Custom indicator (replaces the default dot) |

## Usage (from `packages/core/stories/data/DzTimelineItem.stories.ts`, story `Default`)
...
Editable, running the **All Tones** story from `packages/core/stories/data/DzTimelineItem.stories.ts` (lines 75–91) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzTimelineItem" />

## Variants and controlled state

**Not declared.** `DzTimelineItem` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzTimelineItem` has no `DzTimelineItem.anatomy.ts`, so
...
**Not declared.** `DzTimelineItem` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzTimelineItem` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzTimelineItem` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| Props | 6 | 6 | 1 declare a default, of which 1 declare `undefined` (ADR-20 provider supplies the value) |
