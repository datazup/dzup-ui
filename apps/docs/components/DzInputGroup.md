---
title: "DzInputGroup"
description: "Compound wrapper for input + addons."
outline: [2, 3]
---
...
Compound wrapper for input + addons.

- **Family:** Inputs
- **Source:** `packages/core/src/components/inputs/DzInputGroup.vue` · types `packages/core/src/components/inputs/DzInputGroup.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzInputGroup } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/inputs`
- **Risk tier:** A · **Status:** stable
- **Taxonomy:** size: `icon` `xs` `sm` `md` `lg` `xl`
- **Anatomy parts (ADR-19):** `content`, `prefix`, `root`, `suffix`

## Intent and selection guidance

**Not declared.** `DzInputGroup` declares no `@intent` block in its source header, so
...
| `ui` | `Partial<Record<"root" \| "content" \| "suffix" \| "prefix", DzClassValue>> \| undefined` | no | `undefined` | Per-part class overrides, keyed by the names in `DzInputGroup.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; `ui` addresses the other parts by name, and a typo is a type error. |

## Slots (3)

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | — | Input element and addon children |
| `prefix` | — | Addon content before the input |
| `suffix` | — | Addon content after the input |

## Usage (from `packages/core/stories/inputs/DzInputGroup.stories.ts`, story `Default`)
...
Editable, running the **Size Gallery** story from `packages/core/stories/inputs/DzInputGroup.stories.ts` (lines 89–118) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzInputGroup" />
...
`packages/core/stories/inputs/DzInputGroup.stories.ts`: `Default`, `Size Gallery`, `With Text Prefix`, `With Text Suffix`, `With Prefix and Suffix`, `With Icon Addons`, `With Button Addon`, `States`, `Disabled`, `Dark Mode Preview`, `Interactive`, `Accessibility: Labels & Groups`, `Real World: URL Input`, `Real World: Pricing Input`.
...
```vue
<DzInputGroup :ui="{ 'content': 'ring-2', 'prefix': 'ring-2', 'root': 'ring-2', 'suffix': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/inputs/DzInputGroup.anatomy.ts`.
...
`DzInputGroup` advertises 1 state:
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/inputs/DzInputGroup.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/inputs/DzInputGroup.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/inputs/DzInputGroup.stories.ts` |
