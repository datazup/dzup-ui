...
**Not declared.** `DzTextarea` declares no `@intent` block in its source header, so
...
| `modelValue` | `string \| undefined` | no | `""` | The field's text value; the default empty string renders an empty textarea. |
...
| `ui` | `Partial<Record<"root" \| "error" \| "spinner" \| "input", DzClassValue>> \| undefined` | no | `undefined` | Per-part class overrides, keyed by the names in `DzTextarea.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; `ui` addresses the other parts by name, and a typo is a type error. |
...
<DzTextarea v-model="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzTextarea :modelValue="value" @update:modelValue="value = $event" />
```

**Where each variant is shown.** 16 stories in
`packages/core/stories/inputs/DzTextarea.stories.ts`: `Default`, `Variant Gallery`, `Size Gallery`, `Auto Resize`, `Row Configurations`, `States`, `Loading`, `Disabled`, `Invalid with Error Message`, `With Max Length`, `Dark Mode Preview`, `Interactive`, `Accessibility: Labels & ARIA`, `Real World: Comment Box`, `Real World: Issue Description`, `Readonly`.
...
```vue
<DzTextarea :ui="{ 'error': 'ring-2', 'input': 'ring-2', 'root': 'ring-2', 'spinner': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/inputs/DzTextarea.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/inputs/DzTextarea.anatomy.ts`.
...
`DzTextarea` advertises 4 states:
...
**Published examples:** `state-stories` is `pass` — `packages/core/stories/inputs/DzTextarea.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 23 | 23 | 10 declare a default, of which 4 declare `undefined` (ADR-20 provider supplies the value) |
...
| `keyboard-spec` | tier B | `excepted` | `packages/core/src/components/inputs/DzTextarea.spec.ts` — The component declares `keyboard: 'none'` — an explicit claim that it has no keyboard behaviour of its own, so there is no key sequence for a spec to assert. |
