---
title: "DzText"
description: "General-purpose text component with semantic element and tone control."
outline: [2, 3]
---
...
General-purpose text component with semantic element and tone control.

- **Family:** Typography
- **Source:** `packages/core/src/components/typography/DzText.vue` · types `packages/core/src/components/typography/DzText.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzText } from '@dzup-ui/core'`
...
**Not declared.** `DzText` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `align` | `TextAlign \| undefined` | no | — | Text alignment |
| `as` | `TextElement \| undefined` | no | `"p"` | HTML element to render. Defaults to 'p'. |
| `id` | `string \| undefined` | no | — | Accessible identifier |
| `size` | `TextSize \| undefined` | no | `"md"` | Text size |
| `tone` | `TextTone \| undefined` | no | `"default"` | Semantic color tone |
| `truncate` | `boolean \| undefined` | no | `false` | Truncate text with ellipsis when overflowing |
| `ui` | `Partial<Record<"root", DzClassValue>> \| undefined` | no | `undefined` | Per-part class overrides, keyed by the names in `DzText.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; `ui` addresses the other parts by name, and a typo is a type error. |
| `weight` | `TextWeight \| undefined` | no | — | Font weight override |
...
## Usage (from `packages/core/stories/typography/DzText.stories.ts`, story `Default`)

```vue
<DzText v-bind="args">The quick brown fox jumps over the lazy dog.</DzText>
```

## Playground

Editable, running the **Size Gallery** story from `packages/core/stories/typography/DzText.stories.ts` (lines 97–110) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzText" />
...
`packages/core/stories/typography/DzText.stories.ts`: `Default`, `Size Gallery`, `Tone Gallery`, `Weight Gallery`, `Element Gallery`, `Alignment`, `Truncation`, `Line Clamp (multi-line)`, `Dark Mode Preview`, `Accessibility: Semantic Elements`, `Composition: Mixed Content`, `Real World: Form Helper Text`, `Real World: Article Preview`.
...
Declared in `packages/core/src/components/typography/DzText.anatomy.ts`.
...
| --- | --- | --- | --- |
| Props | 8 | 8 | 4 declare a default, of which 1 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/typography/DzText.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/typography/DzText.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/typography.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/typography/DzText.stories.ts` |
