---
title: "DzRunStatusBadge"
description: "Run-status pill."
outline: [2, 3]
---
...
- **Family:** Feedback
- **Source:** `packages/core/src/components/feedback/DzRunStatusBadge.vue` · types `packages/core/src/components/feedback/DzRunStatusBadge.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzRunStatusBadge } from '@dzup-ui/core'`
...
**Not declared.** `DzRunStatusBadge` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `size` | `CanonicalSize \| undefined` | no | `"md"` | Badge size — maps to DzBadge size |
| `status` | `DzRunStatus` | yes | — | Canonical run status |

## Slots (1)

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | `{ status: DzRunStatus; label: string; }` | Override the human-readable label (defaults to title-cased status) |

## Usage (from `packages/core/stories/_app-specific/DzRunStatusBadge.stories.ts`, story `Default`)

```vue
<DzRunStatusBadge v-bind="args" />
```

## Playground

Editable, running the **Status Gallery** story from `packages/core/stories/_app-specific/DzRunStatusBadge.stories.ts` (lines 69–82) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzRunStatusBadge" />

## Variants and controlled state

**Not declared.** `DzRunStatusBadge` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzRunStatusBadge` has no `DzRunStatusBadge.anatomy.ts`, so
...
**Not declared.** `DzRunStatusBadge` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzRunStatusBadge` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzRunStatusBadge` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/feedback/DzRunStatusBadge.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/feedback/DzRunStatusBadge.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/_app-specific/DzRunStatusBadge.stories.ts` |
