...
- **Family:** Layout
- **Source:** `packages/core/src/components/layout/DzDivider.vue` · types `packages/core/src/components/layout/DzDivider.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzDivider } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/layout`
- **Risk tier:** A · **Status:** stable

## Intent and selection guidance

**Not declared.** `DzDivider` declares no `@intent` block in its source header, so
...
| `decorative` | `boolean \| undefined` | no | `false` | When true, the divider is purely decorative (role="none") |
| `id` | `string \| undefined` | no | — | Unique element ID (prefer `useId()` from Vue 3.5 when auto-generated) |
| `orientation` | `Orientation \| undefined` | no | `"horizontal"` | Divider orientation |

## Usage (from `packages/core/stories/layout/DzDivider.stories.ts`, story `Default`)
...
Editable, running the **Orientation Gallery** story from `packages/core/stories/layout/DzDivider.stories.ts` (lines 92–117) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzDivider" />

## Variants and controlled state

**Not declared.** `DzDivider` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzDivider` has no `DzDivider.anatomy.ts`, so
...
**Not declared.** `DzDivider` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzDivider` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzDivider` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/layout/DzDivider.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/layout/DzDivider.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/layout.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/layout/DzDivider.stories.ts` |
