---
title: "DzDescriptions"
description: "read-only label/value detail grid."
outline: [2, 3]
---
...
read-only label/value detail grid.

- **Family:** Data display
- **Source:** `packages/core/src/components/data/DzDescriptions.vue` · types `packages/core/src/components/data/DzDescriptions.types.ts`
- **Compound parts:** `DzDescriptionsItem`
- **Install:** `npm i @dzup-ui/core` — then `import { DzDescriptions } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/data`
- **Risk tier:** A · **Status:** beta
- **Taxonomy:** size: `icon` `xs` `sm` `md` `lg` `xl`

## Intent and selection guidance

**Not declared.** `DzDescriptions` declares no `@intent` block in its source header, so
...
| `bordered` | `boolean \| undefined` | no | `false` | Render label/value cells with borders (default false) |
| `columns` | `DescriptionsColumns \| undefined` | no | `3` | Number of columns, or a responsive breakpoint map (default 3) |
| `id` | `string \| undefined` | no | — | Unique element ID (prefer `useId()` from Vue 3.5 when auto-generated) |
| `items` | `DescriptionsItem[] \| undefined` | no | `undefined` | Declarative entries. Omit to compose DzDescriptionsItem children instead. |
| `layout` | `DescriptionsLayout \| undefined` | no | `"horizontal"` | Per-pair layout orientation (default 'horizontal') |
| `size` | `CanonicalSize \| undefined` | no | `"md"` | Component size / density (default 'md') |

## Slots (1)

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | — | DzDescriptionsItem children (used when `items` is not provided) |

## Usage (from `packages/core/stories/data/DzDescriptions.stories.ts`, story `Default`)

```vue
<DzDescriptions v-bind="args" />
```

## Playground

Editable, running the **Responsive Columns** story from `packages/core/stories/data/DzDescriptions.stories.ts` (lines 157–178) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzDescriptions" />

## Compound parts

`DzDescriptions` is used with 1 sub-part, documented here rather than on pages of their own because they are not usable outside it.

### DzDescriptionsItem

a single label/value pair within DzDescriptions.

- **Install:** `npm i @dzup-ui/core` — then `import { DzDescriptionsItem } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/data`
- **Compound part of:** `DzDescriptions`

#### Props (2)

| Prop | Type | Required | Declared default | Description |
| --- | --- | --- | --- | --- |
| `label` | `string \| undefined` | no | `undefined` | Term / field label (omit when using the `label` slot) |
| `span` | `number \| undefined` | no | `1` | Number of columns this entry spans (default 1) |
...
A compound sub-part of `DzDescriptions`; see that component's usage snippet.

## Variants and controlled state

**Not declared.** `DzDescriptions` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzDescriptions` has no `DzDescriptions.anatomy.ts`, so
...
**Not declared.** `DzDescriptions` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzDescriptions` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzDescriptions` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| Props | 9 | 9 | 4 declare a default, of which 1 declare `undefined` (ADR-20 provider supplies the value) |
...
**Compound sub-parts are not matrix rows.** `DzDescriptionsItem` is documented on this page and carries no evidence row of its own. Everything below describes `DzDescriptions`. Whether sub-parts should become rows — some of them own a sink their parent declares — is an open owner decision.
