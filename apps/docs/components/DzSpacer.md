...
- **Family:** Layout
- **Source:** `packages/core/src/components/layout/DzSpacer.vue` · types `packages/core/src/components/layout/DzSpacer.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzSpacer } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/layout`
- **Risk tier:** A · **Status:** stable

## Intent and selection guidance

**Not declared.** `DzSpacer` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `size` | `SpacerSize \| undefined` | no | `"auto"` | Size of the spacer. 'auto' uses flex: 1 to fill available space |

## Usage (from `packages/core/stories/layout/DzSpacer.stories.ts`, story `Default`)
...
Editable, running the **Size Gallery** story from `packages/core/stories/layout/DzSpacer.stories.ts` (lines 66–83) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzSpacer" />

## Variants and controlled state

**Not declared.** `DzSpacer` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzSpacer` has no `DzSpacer.anatomy.ts`, so
...
**Not declared.** `DzSpacer` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzSpacer` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzSpacer` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/layout/DzSpacer.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/layout/DzSpacer.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/layout/DzSpacer.stories.ts` |
