...
**Not declared.** `DzRelativeTime` declares no `@intent` block in its source header, so
...
`packages/core/stories/typography/DzRelativeTime.stories.ts`: `JustNow`, `MinutesAgo`, `AbsoluteMode`, `Localized`, `InFeed`.
...
| Custom property |
| --- |
| `--dz-relative-time-font-size` |

Declared in `packages/core/src/components/typography/DzRelativeTime.anatomy.ts`.
