...
- **Family:** Navigation
- **Source:** `packages/core/src/components/navigation/DzBackTop.vue` · types `packages/core/src/components/navigation/DzBackTop.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzBackTop } from '@dzup-ui/core'`
...
- **Anatomy parts (ADR-19):** `icon`, `root`

## Intent and selection guidance

**Not declared.** `DzBackTop` declares no `@intent` block in its source header, so
...
## Props (11, of which 3 inherited from `@dzup-ui/contracts`)
...
| `ariaLabel` | `string \| undefined` | no | `undefined` | Accessible label for the button. Defaults to `'Back to top'`. The button is icon-only, so an accessible name is always rendered. |
| `ariaLabelledby` | `string \| undefined` | no | — | ID of element that labels this component |
| `duration` | `number \| undefined` | no | `400` | Smooth-scroll duration in milliseconds. `0` (or `prefers-reduced-motion`) jumps instantly. Defaults to `400`. |
| `id` | `string \| undefined` | no | — | Unique element ID (prefer `useId()` from Vue 3.5 when auto-generated) |
| `size` | `CanonicalSize \| undefined` | no | `undefined` | Component size (delegated to DzFab). |
| `target` | `Window \| HTMLElement \| null \| undefined` | no | `undefined` | Scroll container to observe and scroll. Defaults to `window` when omitted. Pass an element ref to anchor the affordance to an inner scroll area. |
| `tone` | `CanonicalTone \| undefined` | no | `undefined` | Semantic color tone (delegated to DzFab). |
| `ui` | `Partial<Record<"icon" \| "root", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzBackTop.anatomy.ts` (ADR-19 §5). Forwarded to the underlying `DzFab`, whose `root` and `icon` are the two nodes this component's DOM is made of. |
| `variant` | `DzFabVariant \| undefined` | no | `undefined` | Visual style variant (delegated to DzFab). |
| `visibilityHeight` | `number \| undefined` | no | `400` | Scroll distance (px) past which the button appears. Defaults to `400`. |
...
| --- | --- | --- |
| `default` | — | Icon content. Overrides the default up-arrow glyph. |

## Usage (from `packages/core/stories/navigation/DzBackTop.stories.ts`, story `Default`)
...
Editable, running the **In scroll container** story from `packages/core/stories/navigation/DzBackTop.stories.ts` (lines 142–165) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzBackTop" />
...
`packages/core/stories/navigation/DzBackTop.stories.ts`: `Default`, `Custom threshold`, `In scroll container`, `Dark Mode Preview`.
...
```vue
<DzBackTop :ui="{ 'icon': 'ring-2', 'root': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/navigation/DzBackTop.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/navigation/DzBackTop.anatomy.ts`.
...
`DzBackTop` advertises 1 state:
`idle`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/navigation/DzBackTop.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 11 | 11 | 2 declare a default, of which 5 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `Enter` | Activate the control, scrolling the page back to the top. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
| `Space` | Activate the control, scrolling the page back to the top. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
...
Record: `e2e/at-matrix/DzBackTop.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/navigation/DzBackTop.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/navigation/DzBackTop.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/navigation/DzBackTop.stories.ts` |
| `ssr-sample` | tier A | **`unrun`** | — |
| `token-contrast` | tier A · corpus-wide | `pass` | `packages/tooling/src/token-checks/intent-text-contrast.ts` — Corpus gate: `yarn validate:tokens` covers every pair in the catalog at once. |
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/navigation/DzBackTop.spec.ts` — The component declares 2 binding(s); the unit spec asserts no key event for `Enter`, `Space`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/navigation/DzBackTop.stories.ts` |
| `controlled-uncontrolled` | tier B | **`unrun`** | The unit spec does not exercise both a controlled and an uncontrolled value path. |
| `browser-play` | tier B | `pass` | `packages/core/stories/navigation/DzBackTop.stories.ts` |
| `rtl-contract` | tier B | `present` | `packages/core/src/components/navigation/DzBackTop.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
...
| `at-manual` | tier B | **`unrun`** | `e2e/at-matrix/DzBackTop.md` — 6 AT/browser pairs, none executed. |
