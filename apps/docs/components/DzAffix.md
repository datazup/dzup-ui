...
**Not declared.** `DzAffix` declares no `@intent` block in its source header, so
...
**Not declared.** `DzAffix` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzAffix` has no `DzAffix.anatomy.ts`, so
...
**Not declared.** `DzAffix` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzAffix` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzAffix` declares no anatomy, so there is no state union to show
examples for.
