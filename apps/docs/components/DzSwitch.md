---
title: "DzSwitch"
description: "Toggle switch using Reka UI (ADR-07)."
outline: [2, 3]
---
...
Toggle switch using Reka UI (ADR-07).

- **Family:** Forms
- **Source:** `packages/core/src/components/forms/DzSwitch.vue` · types `packages/core/src/components/forms/DzSwitch.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzSwitch } from '@dzup-ui/core'`
...
**Not declared.** `DzSwitch` declares no `@intent` block in its source header, so
...
| `modelValue` | `boolean \| undefined` | no | `false` | Whether the switch is on; `false` renders it off. |
| `name` | `string \| undefined` | no | `undefined` | Form field name |
| `required` | `boolean \| undefined` | no | `false` | Whether the switch is required |
| `size` | `CanonicalSize \| undefined` | no | `"md"` | Component size |
| `ui` | `Partial<Record<"root" \| "indicator" \| "control" \| "label", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzSwitch.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target. |
...
## Usage (from `packages/core/stories/forms/DzSwitch.stories.ts`, story `Default`)

```vue
<DzSwitch v-bind="args">Enable notifications</DzSwitch>
```

## Playground

Editable, running the **Size Gallery** story from `packages/core/stories/forms/DzSwitch.stories.ts` (lines 79–93) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzSwitch" />
...
<DzSwitch v-model="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzSwitch :modelValue="value" @update:modelValue="value = $event" />
```

**Where each variant is shown.** 8 stories in
`packages/core/stories/forms/DzSwitch.stories.ts`: `Default`, `Size Gallery`, `Disabled`, `States`, `Dark Mode Preview`, `Interactive`, `Accessibility: Focus States`, `Real World: Settings Panel`.
...
```vue
<DzSwitch :ui="{ 'control': 'ring-2', 'indicator': 'ring-2', 'label': 'ring-2', 'root': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/forms/DzSwitch.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/forms/DzSwitch.anatomy.ts`.
...
`DzSwitch` advertises 4 states:
`checked`, `disabled`, `required`, `unchecked`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/forms/DzSwitch.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 11 | 11 | 4 declare a default, of which 6 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `Space` | Toggle the switch. | `2.1.1` | [`switch`](https://www.w3.org/WAI/ARIA/apg/patterns/switch/) |
| `Enter` | Toggle the switch — the underlying element is a button, so Enter activates it. | `2.1.1` | [`switch`](https://www.w3.org/WAI/ARIA/apg/patterns/switch/) |

Declared in `packages/core/src/components/forms/DzSwitch.anatomy.ts`.

- **Pattern:** [APG — `switch`](https://www.w3.org/WAI/ARIA/apg/patterns/switch/) · its *Keyboard Interaction* section is
  the contract this component is audited against.
- **Measured:** `keyboard-spec` is **unrun** — The component declares 2 binding(s); the unit spec asserts no key event for `Space`, `Enter`. The contract is the yardstick, not the presence of any key at all.
...
Record: `e2e/at-matrix/DzSwitch.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/forms/DzSwitch.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/forms/DzSwitch.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/forms.a11y.spec.ts` · `packages/core/tests/a11y/inputs.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/forms/DzSwitch.stories.ts` |
...
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/forms/DzSwitch.spec.ts` — The component declares 2 binding(s); the unit spec asserts no key event for `Space`, `Enter`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/forms/DzSwitch.stories.ts` |
| `controlled-uncontrolled` | tier B | **`unrun`** | The unit spec does not exercise both a controlled and an uncontrolled value path. |
| `browser-play` | tier B | `pass` | `packages/core/stories/forms/DzSwitch.stories.ts` |
| `rtl-contract` | tier B | `present` | `packages/core/src/components/forms/DzSwitch.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
...
| `at-manual` | tier B | **`unrun`** | `e2e/at-matrix/DzSwitch.md` — 6 AT/browser pairs, none executed. |
