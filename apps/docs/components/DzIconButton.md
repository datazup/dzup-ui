...
- **Family:** Buttons
- **Source:** `packages/core/src/components/buttons/DzIconButton.vue` · types `packages/core/src/components/buttons/DzIconButton.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzIconButton } from '@dzup-ui/core'`
...
**Not declared.** `DzIconButton` declares no `@intent` block in its source header, so
...
| `ui` | `Partial<Record<"root" \| "spinner", DzClassValue>> \| undefined` | no | `undefined` | Per-part class overrides, keyed by the names in `DzIconButton.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; `ui` addresses the other parts by name, and a typo is a type error. |
...
## Usage (verbatim story source from `packages/core/stories/buttons/DzIconButton.stories.ts`, story `Default`)
...
Editable, running the **Size Gallery** story from `packages/core/stories/buttons/DzIconButton.stories.ts` (lines 118–150) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzIconButton" />
...
`packages/core/stories/buttons/DzIconButton.stories.ts`: `Default`, `Size Gallery`, `Variant Gallery`, `Tone Gallery`, `States`, `Loading`, `Disabled`, `Dark Mode Preview`, `Interactive: Toggle`, `Real World: Toolbar`, `Real World: Pagination`, `Real World: Close Button`, `Accessibility: Focus States`.
...
```vue
<DzIconButton :ui="{ 'root': 'ring-2', 'spinner': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/buttons/DzIconButton.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/buttons/DzIconButton.anatomy.ts`.
...
`DzIconButton` advertises 3 states:
...
**Published examples:** `state-stories` is `pass` — `packages/core/stories/buttons/DzIconButton.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 10 | 10 | 3 declare a default, of which 4 declare `undefined` (ADR-20 provider supplies the value) |
...
Record: `e2e/at-matrix/DzIconButton.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/buttons/DzIconButton.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/buttons/DzIconButton.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/buttons.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/buttons/DzIconButton.stories.ts` |
...
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/buttons/DzIconButton.spec.ts` — The component declares 2 binding(s); the unit spec asserts no key event for `Enter`, `Space`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/buttons/DzIconButton.stories.ts` |
| `controlled-uncontrolled` | tier B | **`unrun`** | The unit spec does not exercise both a controlled and an uncontrolled value path. |
| `browser-play` | tier B | `pass` | `packages/core/stories/buttons/DzIconButton.stories.ts` |
| `rtl-contract` | tier B | `present` | `packages/core/src/components/buttons/DzIconButton.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
...
| `at-manual` | tier B | **`unrun`** | `e2e/at-matrix/DzIconButton.md` — 6 AT/browser pairs, none executed. |
