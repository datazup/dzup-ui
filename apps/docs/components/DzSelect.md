---
title: "DzSelect"
description: "Select dropdown using Reka UI (ADR-07)."
outline: [2, 3]
---
...
Select dropdown using Reka UI (ADR-07).

- **Family:** Forms
- **Source:** `packages/core/src/components/forms/DzSelect.vue` · types `packages/core/src/components/forms/DzSelect.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzSelect } from '@dzup-ui/core'`
...
- **Anatomy parts (ADR-19):** `content`, `empty`, `error`, `icon`, `input`, `item`, `item-indicator`, `item-label`, `options-message`, `options-retry`, `options-state`, `root`, `trigger`, `viewport`

## Intent and selection guidance

**Not declared.** `DzSelect` declares no `@intent` block in its source header, so
...
## Props (27, of which 14 inherited from `@dzup-ui/contracts`)
...
| `filterFn` | `((option: DzSelectItem, query: string) => boolean) \| undefined` | no | `undefined` | Custom filter function — overrides default case-insensitive label match |
...
| `noResultsText` | `string \| undefined` | no | `undefined` | Text shown when no options match the search query |
...
| `placeholder` | `string \| undefined` | no | `undefined` | Placeholder text shown when no value is selected |
...
| `searchable` | `boolean \| undefined` | no | `false` | Enable search/filter input in the dropdown |
| `searchPlaceholder` | `string \| undefined` | no | `undefined` | Placeholder text for the search input |
| `size` | `CanonicalSize \| undefined` | no | `"md"` | Component size |
| `ui` | `Partial<Record<"icon" \| "root" \| "item" \| "trigger" \| "content" \| "error" \| "item-label" \| "viewport" \| "item-indicator" \| "empty" \| "input" \| "options-state" \| "options-message" \| "options-retry", DzClassValue>> \| undefined` | no | `undefined` | Per-part class overrides, keyed by the names in `DzSelect.anatomy.ts` (ADR-19). `class` keeps landing on the `trigger`, the node it has always landed on; `ui.root` reaches the outer wrapper, and `ui.content` reaches the portaled listbox that no `class` could ever reach. |
...
| `options-state` | `{ state: AsyncOptionsState; message: string; error: string \| undefined; retry: () => void; }` | The single row shown instead of the list while the option set is loading, empty, or failed (renderer contract C9). Given the resolved state, the message the control would have shown, the host's error string, and a `retry` to call. Overriding it replaces all three states at once, on purpose: a consumer who styles "loading" and forgets "error" ships a panel that says nothing when a load fails. |
| `trigger` | `{ value: string \| undefined; placeholder: string \| undefined; }` | Custom trigger content |

## Usage (from `packages/core/stories/forms/DzSelect.stories.ts`, story `Default`)

```vue
<DzSelect v-bind="args" />
```
...
```vue
<DzSelect :ui="{ 'content': 'ring-2', 'empty': 'ring-2', 'error': 'ring-2', 'icon': 'ring-2', 'input': 'ring-2', 'item': 'ring-2', 'item-indicator': 'ring-2', 'item-label': 'ring-2', 'options-message': 'ring-2', 'options-retry': 'ring-2', 'options-state': 'ring-2', 'root': 'ring-2', 'trigger': 'ring-2', 'viewport': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/forms/DzSelect.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/forms/DzSelect.anatomy.ts`.
...
`DzSelect` advertises 7 states:
`checked`, `closed`, `disabled`, `idle`, `invalid`, `open`, `unchecked`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/forms/DzSelect.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 27 | 27 | 10 declare a default, of which 16 declare `undefined` (ADR-20 provider supplies the value) |
...
| `Space` | — | Select the highlighted option when the list is open, otherwise open it. | `2.1.1` | [`combobox`](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/) |
| any character key | — | Type-ahead: move to the next option whose label starts with that character. | `2.1.1` | [`combobox`](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/) |
...
- **Measured:** `keyboard-spec` is **unrun** — The component declares 9 binding(s); the unit spec asserts no key event for `ArrowDown`, `ArrowUp`, `Home`, `End`, `Enter`, `Escape`, `Tab`. The contract is the yardstick, not the presence of any key at all.
...
Record: `e2e/at-matrix/DzSelect.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/forms/DzSelect.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/forms/DzSelect.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/forms.a11y.spec.ts` · `packages/core/tests/a11y/inputs.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/forms/DzSelect.stories.ts` |
...
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/forms/DzSelect.spec.ts` — The component declares 9 binding(s); the unit spec asserts no key event for `ArrowDown`, `ArrowUp`, `Home`, `End`, `Enter`, `Escape`, `Tab`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/forms/DzSelect.stories.ts` |
...
| `portal-hydration` | trait teleports | `present` | `packages/core/tests/ssr/form-controls-ssr.spec.ts` · `packages/core/tests/ssr/ssr-smoke.spec.ts` |
...
**4 unrun:** `keyboard-spec`, `controlled-uncontrolled`, `data-scenarios`, `at-manual` · **1 stale:** `browser-matrix`. They are named rather than counted, because a total tells a reader nothing about what is missing.
