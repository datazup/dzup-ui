---
title: "DzPageHero"
description: "dark gradient hero band for top-level views: eyebrow,"
outline: [2, 3]
---
...
dark gradient hero band for top-level views: eyebrow,

- **Family:** Layout
- **Source:** `packages/core/src/components/layout/DzPageHero.vue` · types `packages/core/src/components/layout/DzPageHero.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzPageHero } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/layout`
- **Risk tier:** A · **Status:** beta

## Intent and selection guidance

**Not declared.** `DzPageHero` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `description` | `string \| undefined` | no | — | Supporting copy under the heading; the description slot overrides it. |
| `eyebrow` | `string \| undefined` | no | — | Uppercase kicker above the heading. |
| `title` | `string` | yes | — | Main heading, rendered as a level-1 DzHeading with gradient text. |

## Slots (3)

| Slot | Slot props | Description |
| --- | --- | --- |
| `actions` | — | Right-aligned action cluster; buttons get the glass treatment. |
| `description` | — | Rich description content; overrides the description prop. |
| `meta` | — | Meta row under the description (chips, counts, breadcrumbs). |

## Usage (from `packages/core/stories/layout/DzPageHero.stories.ts`, story `Bare`)
...
Every story in `packages/core/stories/layout/DzPageHero.stories.ts` either has a computed template or binds Storybook's `args`, neither of which can be mounted outside Storybook. A playground here would have to be written by hand, and a hand-written example is not evidence of anything.

## Variants and controlled state

**Not declared.** `DzPageHero` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzPageHero` has no `DzPageHero.anatomy.ts`, so
...
**Not declared.** `DzPageHero` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzPageHero` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzPageHero` declares no anatomy, so there is no state union to show
examples for.

**Migration.** Breaking changes to this component are recorded in the repository's changesets
and published in the release notes; nothing is restated here, because a hand-typed migration
note drifts from the release it describes the first time the release changes. This component
last changed at `920b0fc`.
...
- **Component last changed at:** `920b0fc8`
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/layout/DzPageHero.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/layout/DzPageHero.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/layout/DzPageHero.stories.ts` |
