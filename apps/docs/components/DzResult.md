---
title: "DzResult"
description: "Operation result display component."
outline: [2, 3]
---
...
- **Family:** Feedback
- **Source:** `packages/core/src/components/feedback/DzResult.vue` · types `packages/core/src/components/feedback/DzResult.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzResult } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/feedback`
- **Risk tier:** A · **Status:** stable

## Intent and selection guidance

**Not declared.** `DzResult` declares no `@intent` block in its source header, so
...
| `status` | `ResultStatus` | yes | — | Status determines the icon and color |
...
| `icon` | — | Custom icon (overrides status icon) |

## Usage (from `packages/core/stories/feedback/DzResult.stories.ts`, story `Default`)

```vue
<DzResult v-bind="args" />
```

## Playground

Editable, running the **Status Gallery** story from `packages/core/stories/feedback/DzResult.stories.ts` (lines 82–95) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzResult" />

## Variants and controlled state

**Not declared.** `DzResult` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzResult` has no `DzResult.anatomy.ts`, so
...
**Not declared.** `DzResult` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzResult` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzResult` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/feedback/DzResult.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/feedback/DzResult.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/feedback/DzResult.stories.ts` |
