---
title: "DzHeading"
description: "Semantic heading component with independent visual sizing."
outline: [2, 3]
---
...
Semantic heading component with independent visual sizing.

- **Family:** Typography
- **Source:** `packages/core/src/components/typography/DzHeading.vue` · types `packages/core/src/components/typography/DzHeading.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzHeading } from '@dzup-ui/core'`
...
**Not declared.** `DzHeading` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `align` | `HeadingAlign \| undefined` | no | — | Text alignment |
| `id` | `string \| undefined` | no | — | Accessible identifier |
| `level` | `HeadingLevel \| undefined` | no | `2` | Semantic heading level (1-6). Determines which HTML heading element to render. |
| `size` | `HeadingSize \| undefined` | no | — | Visual size, independent of semantic level. Defaults based on level if not specified. |
| `truncate` | `boolean \| undefined` | no | `false` | Truncate text with ellipsis when overflowing |
| `ui` | `Partial<Record<"root", DzClassValue>> \| undefined` | no | `undefined` | Per-part class overrides, keyed by the names in `DzHeading.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; `ui` addresses the other parts by name, and a typo is a type error. |
| `weight` | `HeadingWeight \| undefined` | no | — | Font weight override |
...
## Usage (from `packages/core/stories/typography/DzHeading.stories.ts`, story `Default`)

```vue
<DzHeading v-bind="args">Section Heading</DzHeading>
```

## Playground

Editable, running the **All Heading Levels (h1-h6)** story from `packages/core/stories/typography/DzHeading.stories.ts` (lines 88–103) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzHeading" />
...
`packages/core/stories/typography/DzHeading.stories.ts`: `Default`, `All Heading Levels (h1-h6)`, `Size Gallery`, `Weight Gallery`, `Alignment`, `Truncation`, `Size vs Level Independence`, `Dark Mode Preview`, `Accessibility: Semantic Structure`, `Real World: Page Header`, `Real World: Card Headings`.
...
Declared in `packages/core/src/components/typography/DzHeading.anatomy.ts`.
...
| --- | --- | --- | --- |
| Props | 7 | 7 | 2 declare a default, of which 1 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/typography/DzHeading.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/typography/DzHeading.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/typography.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/typography/DzHeading.stories.ts` |
