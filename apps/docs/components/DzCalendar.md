---
title: "DzCalendar"
description: "full-surface month/week calendar for date selection and"
outline: [2, 3]
---
...
- **Family:** Data display
- **Source:** `packages/core/src/components/data/DzCalendar.vue` · types `packages/core/src/components/data/DzCalendar.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzCalendar } from '@dzup-ui/core'`
...
| `minDate` | `string \| undefined` | no | `undefined` | Minimum selectable date (ISO 8601 string) |
...
| --- | --- | --- |
| `day` | `DzCalendarDaySlotProps` | Per-day cell content (event dots, counts, badges). Defaults to day number. |

## Usage (from `packages/core/stories/data/DzCalendar.stories.ts`, story `Month`)
...
Editable, running the **Dark Mode Preview** story from `packages/core/stories/data/DzCalendar.stories.ts` (lines 258–268) **verbatim**. Edits run in a sandbox; nothing here is saved.
...
Script: `e2e/at-matrix/scripts/DzCalendar.at-script.md` · record: `e2e/at-matrix/DzCalendar.md` (append-only).
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/data/DzCalendar.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/data/DzCalendar.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
...
**5 unrun:** `axe`, `ssr-sample`, `keyboard-spec`, `controlled-uncontrolled`, `at-manual` · **2 stale:** `browser-matrix`, `perf-baseline`. They are named rather than counted, because a total tells a reader nothing about what is missing.
