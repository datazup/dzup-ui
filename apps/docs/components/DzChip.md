---
title: "DzChip"
description: "Closable chip component with tone/variant styling."
outline: [2, 3]
---
...
- **Family:** Data display
- **Source:** `packages/core/src/components/data/DzChip.vue` · types `packages/core/src/components/data/DzChip.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzChip } from '@dzup-ui/core'`
...
**Not declared.** `DzChip` declares no `@intent` block in its source header, so
...
| `closable` | `boolean \| undefined` | no | `false` | Whether the chip can be dismissed/closed |
...
| `ui` | `Partial<Record<"root" \| "close", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzChip.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; a key outside the declared parts is a type error, not a class that lands nowhere. |
| `variant` | `ChipVariant \| undefined` | no | `undefined` | Visual style variant |
...
| `prefix` | — | Content before the label (typically an icon or avatar) |

## Usage (from `packages/core/stories/data/DzChip.stories.ts`, story `Default`)

```vue
<DzChip v-bind="args">Chip</DzChip>
```

## Playground

Editable, running the **Variant Gallery** story from `packages/core/stories/data/DzChip.stories.ts` (lines 89–101) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzChip" />
...
`packages/core/stories/data/DzChip.stories.ts`: `Default`, `Variant Gallery`, `Size Gallery`, `Tone Gallery`, `Closable Chips`, `Disabled`, `With Prefix Slot`, `Dark Mode Preview`, `Accessibility: Focus & Close`, `Real World: Active Filters`, `Real World: Input Tags`, `States`.
...
```vue
<DzChip :ui="{ 'close': 'ring-2', 'root': 'ring-2' }" />
```
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/data/DzChip.anatomy.ts`.
...
**Published examples:** `state-stories` is `pass` — `packages/core/stories/data/DzChip.stories.ts`.
...
Record: `e2e/at-matrix/DzChip.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/data/DzChip.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/data/DzChip.spec.ts` |
