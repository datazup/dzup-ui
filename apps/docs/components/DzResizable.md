...
```vue
<DzResizable :ui="{ 'indicator': 'ring-2', 'panel': 'ring-2', 'root': 'ring-2', 'separator': 'ring-2', 'step-decrease': 'ring-2', 'step-increase': 'ring-2' }" />
```
...
DzResizable drags (drag the separator to resize panes), so SC 2.5.7 applies and was audited.
...
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/layout/DzResizable.spec.ts` — The component declares 7 binding(s); the unit spec asserts no key event for `Home`, `End`, `Enter`. The contract is the yardstick, not the presence of any key at all. |
