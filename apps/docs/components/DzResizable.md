---
title: "DzResizable"
description: "Compound resizable panel root using Reka UI Splitter (ADR-07)."
outline: [2, 3]
---
...
Compound resizable panel root using Reka UI Splitter (ADR-07).

- **Family:** Layout
- **Source:** `packages/core/src/components/layout/DzResizable.vue` · types `packages/core/src/components/layout/DzResizable.types.ts`
- **Compound parts:** `DzResizableHandle`, `DzResizablePanel`
- **Install:** `npm i @dzup-ui/core` — then `import { DzResizable } from '@dzup-ui/core'`
...
**Not declared.** `DzResizable` declares no `@intent` block in its source header, so
...
## Usage (from `packages/core/stories/layout/DzResizable.stories.ts`, story `Default`)
...
Editable, running the **Direction Gallery** story from `packages/core/stories/layout/DzResizable.stories.ts` (lines 112–145) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzResizable" />

## Compound parts

`DzResizable` is used with 2 sub-parts, documented here rather than on pages of their own because they are not usable outside it.

### DzResizableHandle

Resize handle between panels, using Reka UI

- **Install:** `npm i @dzup-ui/core` — then `import { DzResizableHandle } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/layout`
- **Compound part of:** `DzResizable`
...
A compound sub-part of `DzResizable`; see that component's usage snippet.

### DzResizablePanel

Panel child of DzResizable using Reka UI SplitterPanel (ADR-07).

- **Install:** `npm i @dzup-ui/core` — then `import { DzResizablePanel } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/layout`
- **Compound part of:** `DzResizable`
...
`packages/core/stories/layout/DzResizable.stories.ts`: `Default`, `Direction Gallery`, `Three Panels`, `Min/Max Size Constraints`, `Collapsible Panel`, `Disabled`, `Handle Without Visual Indicator`, `Nested Layout`, `Dark Mode Preview`, `Interactive: Layout Change`, `Real World: IDE Layout`, `Accessibility: Keyboard Navigation`, `States`.
...
```vue
<DzResizable :ui="{ 'indicator': 'ring-2', 'panel': 'ring-2', 'root': 'ring-2', 'separator': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/layout/DzResizable.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/layout/DzResizable.anatomy.ts`.
...
`DzResizable` advertises 4 states:
...
**Published examples:** `state-stories` is `pass` — `packages/core/stories/layout/DzResizable.stories.ts`.
...
**DzResizable does not meet SC 2.5.7.** The operation — *drag the separator to resize panes* —
...
- **Single pointer, no dragging (SC 2.5.7):** none — SplitterResizeHandle binds only onFocus/onBlur plus the pointer drag; there is no tap, no double-tap and no stepper control. Resizing by a single pointer requires a drag.
...
- **Why it is not fixed:** The affordance is a design decision with no APG precedent -- the Window Splitter pattern specifies keyboard interaction only, and APG has no column-resize pattern at all. Steppers, tap-to-place and preset cycling each change the component's visual language.
...
Record: `e2e/at-matrix/DzResizable.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/layout/DzResizable.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/layout/DzResizable.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/layout/DzResizable.stories.ts` |
| `ssr-sample` | tier A | **`unrun`** | — |
| `token-contrast` | tier A · corpus-wide | `pass` | `packages/tooling/src/token-checks/intent-text-contrast.ts` — Corpus gate: `yarn validate:tokens` covers every pair in the catalog at once. |
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/layout/DzResizable.spec.ts` — The component declares 7 binding(s); the unit spec asserts no key event for `ArrowRight`, `ArrowLeft`, `ArrowDown`, `ArrowUp`, `Home`, `End`, `Enter`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/layout/DzResizable.stories.ts` |
