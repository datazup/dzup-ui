...
- **Family:** Layout
- **Source:** `packages/core/src/components/layout/DzFlex.vue` · types `packages/core/src/components/layout/DzFlex.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzFlex } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/layout`
- **Risk tier:** A · **Status:** stable

## Intent and selection guidance

**Not declared.** `DzFlex` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `align` | `FlexAlign \| undefined` | no | `undefined` | Align items along the cross axis |
...
| `direction` | `FlexDirection \| undefined` | no | `"row"` | Flex direction |
| `gap` | `LayoutGap \| undefined` | no | `"md"` | Gap between flex items |
| `id` | `string \| undefined` | no | — | Unique element ID (prefer `useId()` from Vue 3.5 when auto-generated) |
| `inline` | `boolean \| undefined` | no | `false` | Whether to use inline-flex instead of flex |
| `justify` | `FlexJustify \| undefined` | no | `undefined` | Justify content along the main axis |
| `wrap` | `boolean \| undefined` | no | `false` | Whether to wrap items |
...
## Usage (verbatim story source from `packages/core/stories/layout/DzFlex.stories.ts`, story `Default`)
...
Editable, running the **Direction Gallery** story from `packages/core/stories/layout/DzFlex.stories.ts` (lines 127–144) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzFlex" />

## Variants and controlled state

**Not declared.** `DzFlex` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzFlex` has no `DzFlex.anatomy.ts`, so
...
**Not declared.** `DzFlex` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzFlex` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzFlex` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/layout/DzFlex.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/layout/DzFlex.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/layout.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/layout/DzFlex.stories.ts` |
