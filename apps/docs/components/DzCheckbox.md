---
title: "DzCheckbox"
description: "Checkbox component using Reka UI (ADR-07)."
outline: [2, 3]
---
...
Checkbox component using Reka UI (ADR-07).

- **Family:** Forms
- **Source:** `packages/core/src/components/forms/DzCheckbox.vue` · types `packages/core/src/components/forms/DzCheckbox.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzCheckbox } from '@dzup-ui/core'`
...
- **Anatomy parts (ADR-19):** `control`, `indicator`, `label`, `root`

## Intent and selection guidance

**Not declared.** `DzCheckbox` declares no `@intent` block in its source header, so
...
| `indeterminate` | `boolean \| undefined` | no | `false` | Indeterminate (mixed) state |
| `modelValue` | `boolean \| undefined` | no | `false` | Whether the box is checked; `false` renders it unchecked. |
| `name` | `string \| undefined` | no | `undefined` | Form field name |
| `required` | `boolean \| undefined` | no | `false` | Whether the checkbox is required |
| `size` | `CanonicalSize \| undefined` | no | `undefined` | Component size |
| `ui` | `Partial<Record<"root" \| "indicator" \| "control" \| "label", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzCheckbox.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; a key outside the declared parts is a type error, not a class that lands nowhere. |
| `value` | `string \| undefined` | no | `undefined` | String value for checkbox groups |
...
| `change` | `[checked: boolean]` | Value changed after user interaction |
...
| --- | --- | --- |
| `default` | — | Label text for the checkbox |

## Usage (from `packages/core/stories/forms/DzCheckbox.stories.ts`, story `Default`)

```vue
<DzCheckbox v-bind="args">Accept terms and conditions</DzCheckbox>
```

## Playground

Editable, running the **Size Gallery** story from `packages/core/stories/forms/DzCheckbox.stories.ts` (lines 90–104) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzCheckbox" />
...
<DzCheckbox v-model="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzCheckbox :modelValue="value" @update:modelValue="value = $event" />
```

**Where each variant is shown.** 10 stories in
`packages/core/stories/forms/DzCheckbox.stories.ts`: `Default`, `Size Gallery`, `Disabled`, `Indeterminate`, `States`, `Dark Mode Preview`, `Multi-line Alignment`, `Interactive`, `Accessibility: Focus States`, `Real World: Terms Agreement`.
...
```vue
<DzCheckbox :ui="{ 'control': 'ring-2', 'indicator': 'ring-2', 'label': 'ring-2', 'root': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/forms/DzCheckbox.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/forms/DzCheckbox.anatomy.ts`.
...
`DzCheckbox` advertises 5 states:
`checked`, `disabled`, `indeterminate`, `required`, `unchecked`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/forms/DzCheckbox.stories.ts`.
...
| --- | --- | --- | --- |
| `Space` | Toggle the checkbox, cycling through the indeterminate state when the control is tri-state. | `2.1.1` | [`checkbox`](https://www.w3.org/WAI/ARIA/apg/patterns/checkbox/) |
...
- **Measured:** `keyboard-spec` is **unrun** — The component declares 1 binding(s); the unit spec asserts no key event for `Space`. The contract is the yardstick, not the presence of any key at all.
...
Record: `e2e/at-matrix/DzCheckbox.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/forms/DzCheckbox.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/forms/DzCheckbox.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/forms.a11y.spec.ts` · `packages/core/tests/a11y/inputs.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/forms/DzCheckbox.stories.ts` |
...
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/forms/DzCheckbox.spec.ts` — The component declares 1 binding(s); the unit spec asserts no key event for `Space`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/forms/DzCheckbox.stories.ts` |
| `controlled-uncontrolled` | tier B | **`unrun`** | The unit spec does not exercise both a controlled and an uncontrolled value path. |
| `browser-play` | tier B | `pass` | `packages/core/stories/forms/DzCheckbox.stories.ts` |
| `rtl-contract` | tier B | `present` | `packages/core/src/components/forms/DzCheckbox.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
...
| `at-manual` | tier B | **`unrun`** | `e2e/at-matrix/DzCheckbox.md` — 6 AT/browser pairs, none executed. |

**3 unrun:** `keyboard-spec`, `controlled-uncontrolled`, `at-manual` · **1 stale:** `browser-matrix`. They are named rather than counted, because a total tells a reader nothing about what is missing.
