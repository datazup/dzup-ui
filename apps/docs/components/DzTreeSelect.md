...
- **Anatomy parts (ADR-19):** `content`, `empty`, `error`, `icon`, `indicator`, `input`, `label`, `options-message`, `options-retry`, `options-state`, `root`, `trigger`

## Intent and selection guidance

**Not declared.** `DzTreeSelect` declares no `@intent` block in its source header, so
...
| `modelValue` | `TreeSelectValue` | no | `undefined` | The selected node key or keys, shaped by `selectionMode`, bound with the contract-conforming default `v-model`. Left `undefined` the component reads the legacy `v-model:value` instead; writes go to both (ADR-16, `useDualModel`). |
...
| `ui` | `Partial<Record<"icon" \| "root" \| "trigger" \| "indicator" \| "content" \| "error" \| "empty" \| "label" \| "input" \| "options-state" \| "options-message" \| "options-retry", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzTreeSelect.anatomy.ts` (ADR-19 §5). The tree itself is a `DzTree`, which is its own anatomy boundary — restyle a node through `DzTree`'s parts, not through this map. |
...
| `update:modelValue` | `[value: TreeSelectValue]` | Emitted when the `v-model` binding changes, with the new value. Synthesised by `defineModel` (ADR-16); `v-model` consumes it for you. |
| `update:value` | `[value: TreeSelectValue]` | Emitted when the `v-model:value` binding changes, with the new value. Synthesised by `defineModel` (ADR-16); `v-model:value` consumes it for you. |
...
**Controlled and uncontrolled — `expandedKeys`.** Both forms are supported, and they are
...
<DzTreeSelect v-model:expandedKeys="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzTreeSelect :expandedKeys="value" @update:expandedKeys="value = $event" />
```
...
<!-- Controlled: you own it, and the component only ever asks. -->
<DzTreeSelect v-model="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzTreeSelect :modelValue="value" @update:modelValue="value = $event" />
```
...
<!-- Controlled: you own it, and the component only ever asks. -->
<DzTreeSelect v-model:value="value" />
...
```vue
<DzTreeSelect :ui="{ 'content': 'ring-2', 'empty': 'ring-2', 'error': 'ring-2', 'icon': 'ring-2', 'indicator': 'ring-2', 'input': 'ring-2', 'label': 'ring-2', 'options-message': 'ring-2', 'options-retry': 'ring-2', 'options-state': 'ring-2', 'root': 'ring-2', 'trigger': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/forms/DzTreeSelect.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/forms/DzTreeSelect.anatomy.ts`.
...
`DzTreeSelect` advertises 11 states:
`checked`, `closed`, `disabled`, `idle`, `indeterminate`, `invalid`, `loading`, `open`, `readonly`, `required`, `unchecked`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/forms/DzTreeSelect.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 29 | 29 | 10 declare a default, of which 14 declare `undefined` (ADR-20 provider supplies the value) |
| Events | 11 | 11 | 9 recovered from the `Dz*Emits` interface · 3 synthesised by `defineModel` |
...
| `ArrowRight` | — | Expand the focused node. | `2.1.1` | [`treeview`](https://www.w3.org/WAI/ARIA/apg/patterns/treeview/) |
| `ArrowLeft` | — | Collapse the focused node, or move to its parent. | `2.1.1` | [`treeview`](https://www.w3.org/WAI/ARIA/apg/patterns/treeview/) |
| `Space` | — | Select the focused node. | `2.1.1` | [`listbox`](https://www.w3.org/WAI/ARIA/apg/patterns/listbox/) |
...
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/forms/DzTreeSelect.spec.ts` — The component declares 10 binding(s); the unit spec asserts no key event for `Home`, `End`, `Escape`, `Tab`, `Space`. The contract is the yardstick, not the presence of any key at all. |
...
| `rtl-contract` | tier B | `present` | `packages/core/src/components/forms/DzTreeSelect.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
