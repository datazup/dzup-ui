---
title: "DzTag"
description: "Categorization tag component with tone/variant styling."
outline: [2, 3]
---
...
Categorization tag component with tone/variant styling.

- **Family:** Data display
- **Source:** `packages/core/src/components/data/DzTag.vue` · types `packages/core/src/components/data/DzTag.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzTag } from '@dzup-ui/core'`
...
- **Anatomy parts (ADR-19):** `close`, `root`

## Intent and selection guidance

**Not declared.** `DzTag` declares no `@intent` block in its source header, so
...
| `closable` | `boolean \| undefined` | no | `false` | Whether the tag can be dismissed/closed |
...
| `ui` | `Partial<Record<"root" \| "close", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzTag.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; a key outside the declared parts is a type error, not a class that lands nowhere. |
...
## Usage (from `packages/core/stories/data/DzTag.stories.ts`, story `Default`)

```vue
<DzTag v-bind="args">Tag</DzTag>
```

## Playground

Editable, running the **Variant Gallery** story from `packages/core/stories/data/DzTag.stories.ts` (lines 91–103) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzTag" />
...
`packages/core/stories/data/DzTag.stories.ts`: `Default`, `Variant Gallery`, `Size Gallery`, `Tone Gallery`, `Closable Tags`, `Disabled`, `With Prefix Slot`, `Dark Mode Preview`, `Accessibility: Focus & Dismiss`, `Real World: Issue Labels`, `Real World: Article Categories`, `States`.
...
```vue
<DzTag :ui="{ 'close': 'ring-2', 'root': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/data/DzTag.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/data/DzTag.anatomy.ts`.
...
**Published examples:** `state-stories` is `pass` — `packages/core/stories/data/DzTag.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 10 | 10 | 2 declare a default, of which 3 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- | --- |
| `Enter` | — | Activate the tag. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
| `Space` | — | Activate the tag. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
| `Backspace` | `removable` | Remove the tag. | `2.1.1` | — *(component-specific)* |
| `Delete` | `removable` | Remove the tag. | `2.1.1` | — *(component-specific)* |
...
- **Measured:** `keyboard-spec` is **unrun** — The component declares 4 binding(s); the unit spec asserts no key event for `Enter`, `Space`, `Backspace`, `Delete`. The contract is the yardstick, not the presence of any key at all.
...
Record: `e2e/at-matrix/DzTag.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/data/DzTag.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/data/DzTag.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/data.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/data/DzTag.stories.ts` |
| `ssr-sample` | tier A | `present` | `packages/core/tests/ssr/ssr-smoke.spec.ts` |
| `token-contrast` | tier A · corpus-wide | `pass` | `packages/tooling/src/token-checks/intent-text-contrast.ts` — Corpus gate: `yarn validate:tokens` covers every pair in the catalog at once. |
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/data/DzTag.spec.ts` — The component declares 4 binding(s); the unit spec asserts no key event for `Enter`, `Space`, `Backspace`, `Delete`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/data/DzTag.stories.ts` |
| `controlled-uncontrolled` | tier B | **`unrun`** | The unit spec does not exercise both a controlled and an uncontrolled value path. |
| `browser-play` | tier B | `pass` | `packages/core/stories/data/DzTag.stories.ts` |
| `rtl-contract` | tier B | `present` | `packages/core/src/components/data/DzTag.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
...
| `at-manual` | tier B | **`unrun`** | `e2e/at-matrix/DzTag.md` — 6 AT/browser pairs, none executed. |
