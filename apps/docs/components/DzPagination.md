---
title: "DzPagination"
description: "Page navigation using Reka UI Pagination primitives (ADR-07)."
outline: [2, 3]
---
...
Page navigation using Reka UI Pagination primitives (ADR-07).

- **Family:** Navigation
- **Source:** `packages/core/src/components/navigation/DzPagination.vue` · types `packages/core/src/components/navigation/DzPagination.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzPagination } from '@dzup-ui/core'`
...
- **Anatomy parts (ADR-19):** `action`, `item`, `list`, `root`, `separator`

## Intent and selection guidance

**Not declared.** `DzPagination` declares no `@intent` block in its source header, so
...
| `modelValue` | `number \| undefined` | no | `1` | The current page number, one-based; defaults to page `1`. |
| `pageSize` | `number \| undefined` | no | `10` | Number of items per page |
| `showEdges` | `boolean \| undefined` | no | `false` | Whether to show first/last page buttons |
| `siblingCount` | `number \| undefined` | no | `1` | Number of visible page numbers around the current page |
| `size` | `CanonicalSize \| undefined` | no | `"md"` | Component size |
| `total` | `number` | yes | — | Total number of items |
| `ui` | `Partial<Record<"root" \| "item" \| "action" \| "list" \| "separator", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzPagination.anatomy.ts` (ADR-19 §5). Every node but the `<nav>` is generated from `total` and `pageSize`, so `ui` is the only route to the list, the page buttons, the edge controls and the ellipsis. |
...
| `change` | `[page: number]` | Page value changed |
...
| --- | --- | --- |
| `first` | — | Custom first button content |
| `last` | — | Custom last button content |
| `next` | — | Custom next button content |
| `prev` | — | Custom previous button content |

## Usage (from `packages/core/stories/navigation/DzPagination.stories.ts`, story `Default`)

```vue
<DzPagination v-bind="args" v-model="page" />
```

## Playground

Editable, running the **Size Gallery** story from `packages/core/stories/navigation/DzPagination.stories.ts` (lines 111–124) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzPagination" />
...
<DzPagination v-model="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzPagination :modelValue="value" @update:modelValue="value = $event" />
```

**Where each variant is shown.** 12 stories in
`packages/core/stories/navigation/DzPagination.stories.ts`: `Default`, `Size Gallery`, `With First/Last Buttons`, `Different Sibling Counts`, `Disabled`, `States`, `Few Pages (No Ellipsis)`, `Dark Mode Preview`, `Interactive`, `Accessibility: Navigation`, `Real World: Table Pagination`, `Real World: Blog Navigation`.
...
```vue
<DzPagination :ui="{ 'action': 'ring-2', 'item': 'ring-2', 'list': 'ring-2', 'root': 'ring-2', 'separator': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/navigation/DzPagination.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/navigation/DzPagination.anatomy.ts`.
...
`DzPagination` advertises 3 states:
`disabled`, `idle`, `selected`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/navigation/DzPagination.stories.ts`.
...
| --- | --- | --- | --- |
| Props | 12 | 12 | 6 declare a default, of which 4 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `Tab` | Move to the next page control; each control is its own tab stop. | `2.1.2` | — *(component-specific)* |
| `Enter` | Go to the focused page. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
| `Space` | Go to the focused page. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
...
Record: `e2e/at-matrix/DzPagination.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| `browser-play` | tier B | `pass` | `packages/core/stories/navigation/DzPagination.stories.ts` |
| `rtl-contract` | tier B | `present` | `packages/core/src/components/navigation/DzPagination.anatomy.ts` · `packages/core/docs/rtl-matrix.md` |
