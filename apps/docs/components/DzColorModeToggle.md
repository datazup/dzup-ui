---
title: "DzColorModeToggle"
description: "light / dark / system theme switch."
outline: [2, 3]
---
...
- **Family:** Navigation
- **Source:** `packages/core/src/components/navigation/DzColorModeToggle.vue` · types `packages/core/src/components/navigation/DzColorModeToggle.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzColorModeToggle } from '@dzup-ui/core'`
...
**Not declared.** `DzColorModeToggle` declares no `@intent` block in its source header, so
...
| `labels` | `DzColorModeToggleLabels \| undefined` | no | `undefined` | Per-mode label overrides for i18n. |
| `showSystem` | `boolean \| undefined` | no | `true` | Include the `'system'` option (follow OS `prefers-color-scheme`). When `false`, the control only switches between explicit light and dark. Defaults to `true`. |
| `size` | `CanonicalSize \| undefined` | no | `"md"` | Component size (delegated to the underlying control). Defaults to `'md'`. |
| `ui` | `Partial<Record<"root", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzColorModeToggle.anatomy.ts` (ADR-19 §5). `class` keeps its existing meaning and its existing target; a key outside the declared parts is a type error, not a class that lands nowhere. |
| `variant` | `DzColorModeToggleVariant \| undefined` | no | `"icon"` | Rendering style. Defaults to `'icon'`. |

## Events (1)

| Event | Payload | Description |
| --- | --- | --- |
| `change` | `[theme: ThemePreference]` | The theme preference changed as a result of user interaction. |

## Slots (1)

| Slot | Slot props | Description |
| --- | --- | --- |
| `icon` | `{ mode: "light" \| "dark" \| "system"; theme: ThemePreference; }` | Override the rendered glyph for a given mode (icon / switch variants). Receives the resolved mode and the active theme preference. |

## Usage (from `packages/core/stories/navigation/DzColorModeToggle.stories.ts`, story `IconCycle`)
...
Editable, running the **Dark Mode Preview** story from `packages/core/stories/navigation/DzColorModeToggle.stories.ts` (lines 158–177) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzColorModeToggle" />
...
`packages/core/stories/navigation/DzColorModeToggle.stories.ts`: `Icon (cycle)`, `Switch`, `Segmented`, `Without system`, `Dark Mode Preview`.
...
```vue
<DzColorModeToggle :ui="{ 'root': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/navigation/DzColorModeToggle.anatomy.ts`.
...
**Measured:** `rtl-contract` is `present` — `packages/core/src/components/navigation/DzColorModeToggle.anatomy.ts`.
...
| --- | --- | --- | --- |
| Props | 10 | 10 | 3 declare a default, of which 1 declare `undefined` (ADR-20 provider supplies the value) |
...
| --- | --- | --- | --- |
| `Enter` | Activate the control, advancing the colour mode. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
| `Space` | Activate the control, advancing the colour mode. | `2.1.1` | [`button`](https://www.w3.org/WAI/ARIA/apg/patterns/button/) |
...
Record: `e2e/at-matrix/DzColorModeToggle.md` (append-only). No executable script exists for this component yet; scripts were authored for the Tier C and D components first.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/navigation/DzColorModeToggle.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/navigation/DzColorModeToggle.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/navigation/DzColorModeToggle.stories.ts` |
| `ssr-sample` | tier A | **`unrun`** | — |
| `token-contrast` | tier A · corpus-wide | `pass` | `packages/tooling/src/token-checks/intent-text-contrast.ts` — Corpus gate: `yarn validate:tokens` covers every pair in the catalog at once. |
| `keyboard-spec` | tier B | **`unrun`** | `packages/core/src/components/navigation/DzColorModeToggle.spec.ts` — The component declares 2 binding(s); the unit spec asserts no key event for `Enter`, `Space`. The contract is the yardstick, not the presence of any key at all. |
| `state-stories` | tier B | `pass` | `packages/core/stories/navigation/DzColorModeToggle.stories.ts` |
