...
- **Family:** Cards
- **Source:** `packages/core/src/components/cards/DzStatCard.vue` · types `packages/core/src/components/cards/DzStatCard.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzStatCard } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/cards`
- **Risk tier:** A · **Status:** stable
- **Anatomy parts (ADR-19):** `description`, `header`, `icon`, `root`, `title`

## Intent and selection guidance

**Not declared.** `DzStatCard` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `description` | `string \| undefined` | no | — | Additional description text |
| `icon` | `Component \| undefined` | no | — | Icon component |
| `id` | `string \| undefined` | no | — | Unique element ID |
| `title` | `string` | yes | — | Stat label/title |
| `trend` | `StatTrend \| undefined` | no | — | Trend direction |
| `trendValue` | `string \| undefined` | no | — | Trend value text (e.g., "+12%") |
| `ui` | `Partial<Record<"icon" \| "root" \| "title" \| "description" \| "header", DzClassValue>> \| undefined` | no | — | Per-part class overrides, keyed by the names in `DzStatCard.anatomy.ts` (ADR-19 §5). `class` keeps its existing target (the root surface); `ui` addresses the other parts by name, and a typo is a type error. |
| `value` | `string \| number` | yes | — | Primary value to display |
| `variant` | `StatCardVariant \| undefined` | no | `"elevated"` | Visual style variant |
...
## Usage (from `packages/core/stories/cards/DzStatCard.stories.ts`, story `Default`)

```vue
<DzStatCard v-bind="args" />
```

## Playground

Editable, running the **Variant Gallery** story from `packages/core/stories/cards/DzStatCard.stories.ts` (lines 97–108) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzStatCard" />
...
`packages/core/stories/cards/DzStatCard.stories.ts`: `Default`, `Variant Gallery`, `Trend Directions`, `With Icon`, `With Custom Slots`, `Dark Mode Preview`, `Accessibility: Semantic Structure`, `Real World: Dashboard KPI Row`.
...
```vue
<DzStatCard :ui="{ 'description': 'ring-2', 'header': 'ring-2', 'icon': 'ring-2', 'root': 'ring-2', 'title': 'ring-2' }" />
```
...
Declared in `packages/core/src/components/cards/DzStatCard.anatomy.ts`.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/cards/DzStatCard.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/cards/DzStatCard.spec.ts` |
| `axe` | tier A | `present` | `packages/core/tests/a11y/cards.a11y.spec.ts` |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/cards/DzStatCard.stories.ts` |
