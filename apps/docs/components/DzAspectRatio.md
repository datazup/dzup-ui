...
- **Family:** Layout
- **Source:** `packages/core/src/components/layout/DzAspectRatio.vue` · types `packages/core/src/components/layout/DzAspectRatio.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzAspectRatio } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/layout`
- **Risk tier:** A · **Status:** stable

## Intent and selection guidance

**Not declared.** `DzAspectRatio` declares no `@intent` block in its source header, so
...
| `ratio` | `number \| undefined` | no | `1` | Aspect ratio as a number (width / height). Default: 1 |

## Slots (1)

| Slot | Slot props | Description |
| --- | --- | --- |
| `default` | — | Content to display within the aspect ratio container |

## Usage (from `packages/core/stories/layout/DzAspectRatio.stories.ts`, story `Default`)
...
Editable, running the **Ratio Gallery** story from `packages/core/stories/layout/DzAspectRatio.stories.ts` (lines 63–87) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzAspectRatio" />

## Variants and controlled state

**Not declared.** `DzAspectRatio` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzAspectRatio` has no `DzAspectRatio.anatomy.ts`, so
...
**Not declared.** `DzAspectRatio` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzAspectRatio` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzAspectRatio` declares no anatomy, so there is no state union to show
examples for.
...
| --- | --- | --- | --- |
| `contract-spec` | tier A | `present` | `packages/core/src/components/layout/DzAspectRatio.contract.spec.ts` |
| `unit-spec` | tier A | `present` | `packages/core/src/components/layout/DzAspectRatio.spec.ts` |
| `axe` | tier A | **`unrun`** | — |
| `story-light-dark` | tier A | `pass` | `packages/core/stories/layout/DzAspectRatio.stories.ts` |
