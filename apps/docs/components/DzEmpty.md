...
- **Family:** Feedback
- **Source:** `packages/core/src/components/feedback/DzEmpty.vue` · types `packages/core/src/components/feedback/DzEmpty.types.ts`
- **Install:** `npm i @dzup-ui/core` — then `import { DzEmpty } from '@dzup-ui/core'`
- **Entry points:** `@dzup-ui/core`, `@dzup-ui/core/feedback`
- **Risk tier:** A · **Status:** stable

## Intent and selection guidance

**Not declared.** `DzEmpty` declares no `@intent` block in its source header, so
...
| --- | --- | --- | --- | --- |
| `description` | `string \| undefined` | no | — | Description text for additional context |
| `icon` | `Component \| undefined` | no | — | Icon component to display |
| `id` | `string \| undefined` | no | — | Unique element ID |
| `title` | `string \| undefined` | no | — | Title text for the empty state |
...
| `default` | — | Custom content (overrides title/description/icon) |
| `icon` | — | Custom icon slot (overrides icon prop) |

## Usage (from `packages/core/stories/feedback/DzEmpty.stories.ts`, story `Default`)
...
Editable, running the **With Icon** story from `packages/core/stories/feedback/DzEmpty.stories.ts` (lines 77–92) **verbatim**. Edits run in a sandbox; nothing here is saved.

<DzPlayground component="DzEmpty" />

## Variants and controlled state

**Not declared.** `DzEmpty` declares no recipe axes in its anatomy and exposes no
...
**Not declared.** `DzEmpty` has no `DzEmpty.anatomy.ts`, so
...
**Not declared.** `DzEmpty` calls no `DzProvider` reader, so **nothing an application
...
**Not declared.** `DzEmpty` declares no `rtl` contract, so this page cannot say
...
**Not declared.** `DzEmpty` declares no anatomy, so there is no state union to show
examples for.
