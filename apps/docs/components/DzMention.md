...
**Not declared.** `DzMention` declares no `@intent` block in its source header, so
...
<DzMention v-model="value" />

<!-- Controlled, long form — the same thing, written out. -->
<DzMention :modelValue="value" @update:modelValue="value = $event" />
```
...
Your `class`, `id` and `data-*` land on the `input` part —
`[data-part="input"]`.
...
The merge order above still holds: your `class` beats `ui.input`.
...
Declared in `packages/core/src/components/forms/DzMention.anatomy.ts`.
...
`DzMention` advertises 6 states:
`active`, `disabled`, `invalid`, `loading`, `readonly`, `required`. Each is emitted as `data-state` or as a
presence-only boolean attribute, so it is selectable in CSS and assertable in a test.

**Published examples:** `state-stories` is `pass` — `packages/core/stories/forms/DzMention.stories.ts`.
...
| --- | --- | --- | --- | --- |
| any character key | — | Typing the trigger character opens the suggestion list. | `2.1.1` | [`combobox`](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/) |
| `ArrowDown` | `list open` | Move to the next suggestion. | `2.1.1` | [`combobox`](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/) |
| `ArrowUp` | `list open` | Move to the previous suggestion. | `2.1.1` | [`combobox`](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/) |
| `Enter` | `list open` | Insert the highlighted suggestion. | `2.1.1` | [`combobox`](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/) |
| `Tab` | `list open` | Insert the highlighted suggestion. | `2.1.2` | [`combobox`](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/) |
| `Escape` | — | Close the suggestion list and keep the typed text. | `2.1.1`, `2.1.2` | [`combobox`](https://www.w3.org/WAI/ARIA/apg/patterns/combobox/) |
