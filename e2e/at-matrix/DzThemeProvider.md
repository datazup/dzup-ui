...
# DzThemeProvider — manual AT task matrix

**Tier B · APG pattern `none` · source `packages/core/src/providers/DzThemeProvider.vue`**
...
`DzThemeProvider` declares `keyboard: 'none'` in `packages/core/src/providers/DzThemeProvider.anatomy.ts` — an explicit claim that it
