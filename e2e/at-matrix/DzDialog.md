...
# DzDialog — manual AT task matrix

**Tier B · APG pattern `dialog` · source `packages/core/src/components/overlays/DzDialog.vue`**
...
`packages/core/src/components/overlays/DzDialog.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
...
| `Tab` | — | Move to the next focusable element, wrapping inside the dialog. |
| `Shift` + `Tab` | — | Move to the previous focusable element, wrapping inside the dialog. |
