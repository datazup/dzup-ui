...
# DzColorPicker — manual AT task matrix
...
`packages/core/src/components/forms/DzColorPicker.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `ArrowRight` | — | Move the colour pointer one step along the saturation axis. |
| `ArrowLeft` | — | Move the colour pointer one step back along the saturation axis. |
| `ArrowUp` | — | Move the colour pointer one step up the value axis. |
| `ArrowDown` | — | Move the colour pointer one step down the value axis. |
| `Home` | — | Move the colour pointer to the start of the axis. |
| `End` | — | Move the colour pointer to the end of the axis. |
