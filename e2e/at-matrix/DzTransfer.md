...
# DzTransfer — manual AT task matrix
...
`packages/core/src/components/forms/DzTransfer.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
...
| `Enter` | transfer action | Move the selected items to the other list. |
