...
`packages/core/src/components/overlays/DzSheet.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Escape` | — | Close the sheet and return focus to the element that opened it. |
| `Tab` | — | Move to the next focusable element, wrapping inside the sheet. |
| `Shift` + `Tab` | — | Move to the previous focusable element, wrapping inside the sheet. |
