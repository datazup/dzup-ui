...
# DzButton — manual AT task matrix

**Tier B · APG pattern `button` · source `packages/core/src/components/buttons/DzButton.vue`**
...
`packages/core/src/components/buttons/DzButton.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
