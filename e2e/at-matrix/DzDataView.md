...
# DzDataView — manual AT task matrix
...
`packages/core/src/components/data/DzDataView.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Tab` | — | Move between the layout controls and the rendered items. |
