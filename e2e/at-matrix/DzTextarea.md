...
# DzTextarea — manual AT task matrix

**Tier B · APG pattern `none` · source `packages/core/src/components/inputs/DzTextarea.vue`**
...
`DzTextarea` declares `keyboard: 'none'` in `packages/core/src/components/inputs/DzTextarea.anatomy.ts` — an explicit claim that it
