...
# DzTooltip — manual AT task matrix

**Tier B · APG pattern `tooltip` · source `packages/core/src/components/overlays/DzTooltip.vue`**
...
`packages/core/src/components/overlays/DzTooltip.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
...
| `Tab` | — | Move focus to the trigger, which shows the tooltip. |
