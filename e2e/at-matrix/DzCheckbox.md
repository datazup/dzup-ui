...
# DzCheckbox — manual AT task matrix

**Tier B · APG pattern `checkbox` · source `packages/core/src/components/forms/DzCheckbox.vue`**
...
`packages/core/src/components/forms/DzCheckbox.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Space` | — | Toggle the checkbox, cycling through the indeterminate state when the control is tri-state. |
