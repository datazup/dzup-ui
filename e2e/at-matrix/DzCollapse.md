...
# DzCollapse — manual AT task matrix

**Tier B · APG pattern `disclosure` · source `packages/core/src/components/layout/DzCollapse.vue`**
...
`packages/core/src/components/layout/DzCollapse.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Enter` | trigger | Expand or collapse the region. |
| `Space` | trigger | Expand or collapse the region. |
