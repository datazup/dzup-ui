...
# DzListbox — manual AT task matrix

**Tier B · APG pattern `listbox` · source `packages/core/src/components/forms/DzListbox.vue`**
...
`packages/core/src/components/forms/DzListbox.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
