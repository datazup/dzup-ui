...
# DzSplitButton — manual AT task matrix

**Tier B · APG pattern `menu-button` · source `packages/core/src/components/buttons/DzSplitButton.vue`**
...
`packages/core/src/components/buttons/DzSplitButton.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Enter` | root | Activate the primary action. |
| `Space` | root | Activate the primary action. |
| `ArrowDown` | trigger | Open the menu and focus its first item. |
| `Enter` | trigger | Open the menu and focus its first item. |
