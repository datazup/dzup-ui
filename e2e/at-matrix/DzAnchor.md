...
# DzAnchor — manual AT task matrix

**Tier B · APG pattern `link` · source `packages/core/src/components/navigation/DzAnchor.vue`**
...
`packages/core/src/components/navigation/DzAnchor.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
