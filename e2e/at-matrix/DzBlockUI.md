...
# DzBlockUI — manual AT task matrix

**Tier B · APG pattern `custom` · source `packages/core/src/components/feedback/DzBlockUI.vue`**
...
`DzBlockUI` declares `keyboard: 'none'` in `packages/core/src/components/feedback/DzBlockUI.anatomy.ts` — an explicit claim that it
