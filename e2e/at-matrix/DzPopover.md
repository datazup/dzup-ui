...
# DzPopover — manual AT task matrix

**Tier B · APG pattern `dialog` · source `packages/core/src/components/overlays/DzPopover.vue`**
...
`packages/core/src/components/overlays/DzPopover.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Escape` | — | Close the popover and return focus to the element that opened it. |
| `Tab` | — | Move to the next focusable element, wrapping inside the popover. |
| `Shift` + `Tab` | — | Move to the previous focusable element, wrapping inside the popover. |
