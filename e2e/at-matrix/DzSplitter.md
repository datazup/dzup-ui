...
# DzSplitter — manual AT task matrix

**Tier B · APG pattern `window-splitter` · source `packages/core/src/components/layout/DzSplitter.vue`**
...
`packages/core/src/components/layout/DzSplitter.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
