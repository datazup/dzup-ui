...
# DzRating — manual AT task matrix

**Tier B · APG pattern `custom` · source `packages/core/src/components/forms/DzRating.vue`**
...
`packages/core/src/components/forms/DzRating.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `ArrowUp` | — | Increase the rating by one step. |
| `ArrowDown` | — | Decrease the rating by one step. |
| `Home` | — | Set the rating to its minimum. |
| `End` | — | Set the rating to its maximum. |
