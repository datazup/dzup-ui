...
# DzDropdownMenu — manual AT task matrix

**Tier B · APG pattern `menu` · source `packages/core/src/components/overlays/DzDropdownMenu.vue`**
...
`packages/core/src/components/overlays/DzDropdownMenu.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
