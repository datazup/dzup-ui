...
# DzSelect — manual AT task matrix

**Tier B · APG pattern `combobox` · source `packages/core/src/components/forms/DzSelect.vue`**
...
`packages/core/src/components/forms/DzSelect.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
...
| `Enter` | — | Select the highlighted option and close the list. |
| `Escape` | — | Close the list without changing the value. |
| `Tab` | — | Move out of the control, closing the list. |
| `Space` | — | Select the highlighted option when the list is open, otherwise open it. |
| any character key | — | Type-ahead: move to the next option whose label starts with that character. |
