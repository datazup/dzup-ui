...
# DzPopconfirm — manual AT task matrix

**Tier B · APG pattern `alertdialog` · source `packages/core/src/components/overlays/DzPopconfirm.vue`**
...
`packages/core/src/components/overlays/DzPopconfirm.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Escape` | — | Dismiss the confirmation without taking the action. |
| `Tab` | — | Move between the actions, wrapping inside the confirmation. |
| `Shift` + `Tab` | — | Move backwards between the actions. |
