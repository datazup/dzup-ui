...
# DzFab — manual AT task matrix

**Tier B · APG pattern `button` · source `packages/core/src/components/buttons/DzFab.vue`**
...
`packages/core/src/components/buttons/DzFab.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Enter` | — | Activate the action. |
| `Space` | — | Activate the action. |
