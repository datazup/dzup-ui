...
# DzStepper — manual AT task matrix

**Tier B · APG pattern `custom` · source `packages/core/src/components/navigation/DzStepper.vue`**
...
`packages/core/src/components/navigation/DzStepper.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Tab` | — | Move to the next navigable step; each step is its own tab stop. |
