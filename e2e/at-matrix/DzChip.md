...
# DzChip — manual AT task matrix

**Tier B · APG pattern `button` · source `packages/core/src/components/data/DzChip.vue`**
...
`packages/core/src/components/data/DzChip.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Enter` | — | Activate the chip. |
| `Space` | — | Activate the chip. |
| `Backspace` | removable | Remove the chip. |
| `Delete` | removable | Remove the chip. |
