...
# DzPasswordInput — manual AT task matrix

**Tier B · APG pattern `none` · source `packages/core/src/components/inputs/DzPasswordInput.vue`**
...
`packages/core/src/components/inputs/DzPasswordInput.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Enter` | toggle | Show or hide the password. |
| `Space` | toggle | Show or hide the password. |
