...
# DzToggleButton — manual AT task matrix

**Tier B · APG pattern `button` · source `packages/core/src/components/buttons/DzToggleButton.vue`**
...
`packages/core/src/components/buttons/DzToggleButton.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
