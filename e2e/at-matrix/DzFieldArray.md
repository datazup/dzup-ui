...
# DzFieldArray — manual AT task matrix

**Tier B · APG pattern `custom` · source `packages/core/src/components/forms/DzFieldArray.vue`**
...
`packages/core/src/components/forms/DzFieldArray.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Enter` | action | Activate the focused add or remove control. |
| `Space` | action | Activate the focused add or remove control. |
