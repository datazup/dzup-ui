...
# DzSwitch — manual AT task matrix

**Tier B · APG pattern `switch` · source `packages/core/src/components/forms/DzSwitch.vue`**
...
`packages/core/src/components/forms/DzSwitch.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
...
| `Enter` | — | Toggle the switch — the underlying element is a button, so Enter activates it. |
