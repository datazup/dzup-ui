...
# DzSlider — manual AT task matrix

**Tier B · APG pattern `slider` · source `packages/core/src/components/forms/DzSlider.vue`**
...
`packages/core/src/components/forms/DzSlider.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
