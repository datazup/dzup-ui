...
# DzTagsInput — manual AT task matrix

**Tier B · APG pattern `custom` · source `packages/core/src/components/forms/DzTagsInput.vue`**
...
`packages/core/src/components/forms/DzTagsInput.anatomy.ts` and are the same rows the component's documentation page publishes. A key that
does not do what this table says is a defect in the component **or** in the contract — record which,
in a note.

| Key | Where | Must |
|---|---|---|
| `Enter` | — | Commit the pending token as a tag. |
| `Backspace` | input empty | Remove the last tag when the field is empty. |
| any character key | — | A configured delimiter character commits the pending token. |
