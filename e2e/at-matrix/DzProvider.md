...
# DzProvider — manual AT task matrix

**Tier B · APG pattern `none` · source `packages/core/src/providers/DzProvider.vue`**
...
`DzProvider` declares `keyboard: 'none'` in `packages/core/src/providers/DzProvider.anatomy.ts` — an explicit claim that it
