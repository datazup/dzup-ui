...
| **System foundations 08-11** — `NEXT_SESSION_PROMPT_2026-08-11-dzup-ui-system-foundations.md` | Executed as TASK-OSS-P0-01/02 + P1-01…04 in August: exact-name ownership manifest, resolver without prefix heuristics, Nuxt module naming `@dzup-ui-pro/pro` (the only `@dzup-ui/pro` left in `packages/core/src` is a negative assertion in `resolver.spec.ts`). Nothing remains. | <!-- retired-name-ok: records the rename that retired it -->
