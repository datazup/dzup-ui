...
Documenting the 436 names removes the `type-only export not listed in
public-api.manifest.json#exports.types` evidence notes (304) from the generated
`component-ownership.manifest.json`. Classifications are unchanged. The
generator's spec asserted that this drift exists. Added paths:

- `packages/core/manifests/component-ownership.manifest.json` (regenerated by `yarn generate:ownership:core`)
- `packages/tooling/src/ownership/generate-ownership-manifest.spec.ts` (drift assertion inverted)

### Amendment 2 (new epoch, after landing 631bc74)

CI run 36121587729 on 631bc74 failed one step: Storybook Build, "Playground
snippets in sync with component sources". `build-playground-snippets.mjs`
enumerates components from the manifest's family lists, so the 43 newly
documented components now get snippets (164 → 207, additions only). This step
is not part of `validate:all`. Added path:

- `apps/storybook/stories/_data/playgroundSnippets.generated.ts` (regenerated by `yarn workspace @dzup-ui/storybook build:playground-snippets`)
