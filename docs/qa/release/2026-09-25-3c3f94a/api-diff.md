...
> Generated 2026-09-25T06:44:35.879Z by `release:api-diff`.
...
### `@dzup-ui/core` (`packages/core/src/index.ts`) — **clean**

`yarn generate:exports` would neither drop nor add an export line (comments and order are not compared).
...
- **436 exported and undocumented** — by kind: 43 component, 26 const, 63 function, 212 interface, 92 type
- **0 documented and undelivered**
...
### `@dzup-ui/core` — 436 change(s), fidelity `manifest-only`
...
| added | `DzQRStatus` | additive | patch | new type on ., ./media |
| added | `DzRating` | additive | patch | new component on ., ./forms |
| … | _236 more — see the JSON_ | | | |
