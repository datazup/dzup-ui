...
export function registerTools(server: McpServer, client: RegistryClient): void {
  ...
  const guard = <A>(fn: (a: A) => Promise<ToolResult>) => async (args: A) => {
    try {
      return text(await fn(args))
    }
    catch (err) {
      const message = err instanceof Error ? err.message : String(err)
      return { content: [{ type: 'text' as const, text: `Failed to reach the dzup-ui registry (${client.base}): ${message}` }], isError: true }
    }
  }

  server.registerTool('list_components', {
    title: 'List components',
    description: 'List @dzup-ui/core components with a one-line summary, optionally filtered by family (e.g. "Buttons", "Forms") or a free-text query. Start here to discover what exists.',
    inputSchema: {
  ...
  server.registerTool('get_component', {
    ...
    description: 'Get the full public API for one component: import path, frozen variant/size/tone taxonomy, props/emits/slots tables and a usage snippet.',
    inputSchema: {
  ...
  server.registerTool('list_blocks', {
    title: 'List blocks',
    description: 'List pre-composed UI blocks (heroes, pricing, navbars, forms…) built from @dzup-ui/core. Filter by category or free-text query.',
    inputSchema: {
      category: z.string().optional().describe('Block category, e.g. "marketing", "forms", "commerce"'),
      query: z.string().optional().describe('Free-text filter over name/title/description/components'),
    },
  }, guard((a: { category?: string, query?: string }) => listBlocks(client, a)))

  server.registerTool('get_block', {
    title: 'Get block source',
    description: 'Fetch a block\'s real .vue source, its npm dependencies and the `shadcn add` install command. Use the result to drop the block into a project.',
    inputSchema: {
  ...
  server.registerTool('list_templates', {
    ...
    description: 'List full-page templates (dashboards, auth pages, settings, marketing pages…). Filter by category or free-text query.',
    inputSchema: {
      category: z.string().optional().describe('Template category, e.g. "dashboards", "auth", "marketing"'),
      query: z.string().optional().describe('Free-text filter over name/title/description/components'),
    },
  ...
  server.registerTool('get_template', {
    ...
    description: 'Fetch a full-page template\'s real .vue source, its npm dependencies and the `shadcn add` install command.',
    inputSchema: {
  ...
  server.registerTool('list_tokens', {
    ...
    description: 'List the --dz-* design tokens (OKLCH colors, spacing, radii, shadows…) as light/dark CSS variables. Filter by theme or a name/value query.',
    inputSchema: {
      theme: z.enum(['light', 'dark']).optional().describe('Restrict to one theme; omit for both'),
      query: z.string().optional().describe('Filter tokens by name or value, e.g. "primary", "radius"'),
    },
  ...
  server.registerTool('get_install_command', {
    ...
    description: 'Get the exact terminal commands to install a block, template or the token theme into a project, for a chosen package manager.',
    inputSchema: {
      name: z.string().describe('Block/template name (ignored for type="tokens")'),
      type: z.enum(['block', 'template', 'tokens']).optional().describe('What to install (default: block)'),
      packageManager: z.enum(PACKAGE_MANAGERS).optional().describe('npm | pnpm | yarn | bun (default: npm)'),
    },
  }, guard((a: { name: string, type?: 'block' | 'template' | 'tokens', packageManager?: (typeof PACKAGE_MANAGERS)[number] }) => getInstallCommand(client, a)))

  server.registerTool('search', {
    title: 'Search catalog',
    description: 'One free-text search across components, blocks and templates at once. Returns the top matches in each category with the tool to call next.',
    inputSchema: {
      query: z.string().describe('What you are looking for, e.g. "pricing", "date picker", "sidebar"'),
    },
...
export function createServer(client = new RegistryClient()): McpServer {
  const server = new McpServer({ name: 'dzup-ui', version: VERSION }, { instructions: INSTRUCTIONS })
...
async function main(): Promise<void> {
  ...
  console.error(`dzup-ui MCP server running (registry: ${client.base}, default: ${DEFAULT_REGISTRY_URL})`)
}
