      ...
      <div class="mx-auto max-w-6xl px-6 py-8">
        ...
        <section class="mt-7 grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-4">
          ...
          >
            <DzCardBody class="flex h-full flex-col p-6">
              ...
              <span class="mt-3 text-3xl font-semibold leading-none tracking-tight tabular-nums">
                {{ kpi.value }}
              </span>
        ...
        <DzCard variant="elevated" padding="none" class="mt-6">
          ...
          <ul class="divide-y divide-[var(--dz-border)]">
            <li v-for="row in recent" :key="row.name" class="flex items-center gap-3 px-6 py-3.5">
              <span
                class="grid h-9 w-9 shrink-0 place-items-center rounded-[var(--dz-radius-full)] bg-[var(--dz-primary-muted)] text-xs font-semibold tabular-nums text-[var(--dz-primary-muted-foreground)]"
                aria-hidden="true"
              >
                {{ row.initials }}
              </span>
