...
const resolvedVariant = computed(
  () => resolve<BadgeVariant>('DzTag', 'variant', [props.variant]) ?? 'subtle',
)

/** Resolved size: prop, then provider, then default */
const resolvedSize = computed(
  () => resolve<CanonicalSize>('DzTag', 'size', [props.size]) ?? 'md',
)

/** Resolved tone: prop, then provider, then default */
const resolvedTone = computed(
  () => resolve<CanonicalTone>('DzTag', 'tone', [props.tone]) ?? 'neutral',
)
...
<template>
  <span
    ...
    v-bind="{ ...dzTestId('dz-tag'), ...$attrs, class: undefined }"
    @focus="handleFocus"
    @blur="handleBlur"
  ...
  >
    ...
    <button
      ...
      :class="[
        resolvedSize === 'sm' ? 'h-3 w-3 [--dz-control-visual-size:0.75rem]' : '',
        resolvedSize === 'md' ? 'h-3.5 w-3.5 [--dz-control-visual-size:0.875rem]' : '',
        resolvedSize === 'lg' ? 'h-4 w-4 [--dz-control-visual-size:1rem]' : '',
        ui?.close,
      ]"
