...
/**
 * DzSplitter — declared anatomy for the splitter family (TASK-R5-O2, ADR-19).
 *
 ...
 * `DzSplitterPanel` and `DzSplitterHandle`, which the ownership manifest
 * records as `compound-part`s with `parentComponent: 'DzSplitter'`. They emit
 * `panel`, `separator` and `indicator` — never `root` — so a composed
 * splitter conforms to this one declaration (N2-S1 finding S1-F3, the
...
export const anatomy = {
  ...
  /**
   * Empty and measured: `DzSplitter.variants.ts` reads global semantic tokens
   * (`--dz-border`, `--dz-primary`, `--dz-control-visual-size`) and owns no
   * `--dz-splitter-*` property.
   */
  componentTokens: [],
...
/** Addressable node names across the splitter family. */
export type DzSplitterPart = AnatomyPart<typeof anatomy>

/**
 * `ui` prop shape for DzSplitter — the group is the only node it renders.
 *
 * `panel`, `separator` and `indicator` are written by the consumer at the
 * call site and already take `class`.
 */
export type DzSplitterUi = Pick<UiOverrides<typeof anatomy>, 'root'>
